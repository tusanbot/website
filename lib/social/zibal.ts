...
function merchant() {
    const value = process.env.ZIBAL_MERCHANT;
    if (!value) throw new Error("ZIBAL_MERCHANT is not configured");
    return value;
}

async function requestJson(path: string, body: Record<string, unknown>) {
    const response = await fetch(`${ZIBAL_API}/${path}`, {
        method: "POST",
...
export async function requestZibalPayment(input: {
    ...
    if (!Number.isSafeInteger(trackId) || trackId <= 0) {
        throw new Error(typeof data.message === "string" ? data.message : "درگاه زیبال شناسه پرداخت معتبر برنگرداند.");
    }
...
export async function verifyZibalPayment(trackId: number) {
    ...
    return {
        success: Number(data.result) === 100,
        result: Number(data.result),
        message: typeof data.message === "string" ? data.message : "",
        amount: Number(data.amount),
        referenceNumber: data.refNumber == null ? null : String(data.refNumber),
        raw: data,
    };
}

export function zibalStartUrl(trackId: number) {
    return `https://gateway.zibal.ir/start/${encodeURIComponent(String(trackId))}`;
}
