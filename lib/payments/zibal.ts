...
export class ZibalGateway implements PaymentGateway {
  ...
  async createPayment(input: CreateGatewayPaymentInput): Promise<GatewayPaymentResult> {
    ...
    if (!trackId || Number(data?.result) !== 100) {
      throw new Error(data?.message || "درخواست پرداخت از زیبال ایجاد نشد.");
    }
  ...
  async verifyPayment(authority: string, amount: number): Promise<VerifyGatewayPaymentResult> {
    ...
    if (Number(data?.result) !== 100) {
      return {
        success: false,
        raw: data,
        message: data?.message || "پرداخت زیبال تأیید نشد.",
      };
    }
    ...
    return {
      success: true,
      transactionId: data?.refNumber != null ? String(data.refNumber) : undefined,
      raw: data,
    };
  }
}
