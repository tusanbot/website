...
export function matchesPricingRule(rule: PricingRule, data: Record<string, unknown>): boolean {
  ...
  switch (rule.operator) {
    case "equals":
      return Array.isArray(actual) ? actual.some((item) => normalize(item) === normalize(expected)) : normalize(actual) === normalize(expected);
    case "not_equals":
      return Array.isArray(actual) ? !actual.some((item) => normalize(item) === normalize(expected)) : normalize(actual) !== normalize(expected);
    case "contains":
      return Array.isArray(actual) ? actual.some((item) => normalize(item).toLowerCase().includes(normalize(expected).toLowerCase())) : normalize(actual).toLowerCase().includes(normalize(expected).toLowerCase());
    case "not_contains":
      return Array.isArray(actual) ? !actual.some((item) => normalize(item).toLowerCase().includes(normalize(expected).toLowerCase())) : !normalize(actual).toLowerCase().includes(normalize(expected).toLowerCase());
...
export function calculateServicePrice(basePrice: number, rules: PricingRule[], data: Record<string, unknown>): number {
  ...
  const enabledRules = (rules || []).filter((rule) => rule.enabled !== false && Number.isFinite(Number(rule.amount)));
  for (const rule of enabledRules) {
    if (rule.mode === "set" && matchesPricingRule(rule, data)) price = Math.max(0, Number(rule.amount));
  }
  for (const rule of enabledRules) {
