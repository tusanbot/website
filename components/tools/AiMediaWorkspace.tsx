...
export default function AiMediaWorkspace({ mode, title, description }: Props) {
  ...
  async function generate() {
    ...
    try {
      ...
      if (reference) {
        if (reference.size > 8 * 1024 * 1024) throw new Error("حجم تصویر مرجع نباید بیشتر از ۸ مگابایت باشد.");
  ...
  return <div className="grid gap-4 lg:grid-cols-[minmax(0,1fr)_minmax(300px,0.85fr)]" dir="rtl">
    <div className="rounded-3xl border border-[var(--border)] bg-[var(--surface)] p-4 sm:p-5">
      ...
      <textarea value={prompt} onChange={(e) => setPrompt(e.target.value)} rows={8} placeholder={mode === "image" ? "مثلاً: یک پوستر حرفه‌ای برای کافی‌نت توسن، سبک مدرن، نورپردازی استودیویی..." : mode === "video" ? "صحنه، حرکت دوربین، سوژه، نور، سبک و نسبت تصویر را دقیق توضیح دهید..." : "سبک موسیقی، سازها، حال‌وهوا، سرعت، خواننده یا بی‌کلام بودن و مدت تقریبی را بنویسید..."} className="mt-4 w-full resize-y rounded-2xl border border-[var(--border)] bg-[var(--surface-secondary)] px-4 py-3 text-sm leading-7 outline-none focus:border-[var(--primary)]" />
      <div className="mt-3 grid gap-3 sm:grid-cols-2">
        <label className="text-xs font-bold">نسبت تصویر/ویدیو<select value={aspectRatio} onChange={(e) => setAspectRatio(e.target.value)} className="mt-2 w-full rounded-xl border border-[var(--border)] bg-[var(--surface-secondary)] px-3 py-2 text-sm"><option>1:1</option><option>16:9</option><option>9:16</option><option>4:3</option><option>3:4</option></select></label>
        {(mode === "image" || mode === "music") && <label className="text-xs font-bold">تصویر مرجع (اختیاری)<input type="file" accept={accept} onChange={(e) => setReference(e.target.files?.[0] || null)} className="mt-2 block w-full text-xs" /></label>}
      </div>
      {mode === "video" && <div className="mt-3 rounded-xl bg-[var(--surface-secondary)] p-3 text-xs leading-6 text-[var(--text-muted)]">ویدیوهای Veo فعلاً کوتاه هستند و با صدای تولیدشده توسط مدل ارائه می‌شوند.</div>}
      {error && <div className="mt-3 rounded-xl border border-red-500/20 bg-red-500/5 p-3 text-xs leading-6 text-red-600">{error}</div>}
      {status && !error && <div className="mt-3 rounded-xl bg-[var(--primary)]/5 p-3 text-xs leading-6 text-[var(--text-muted)]">{status}</div>}
      <button type="button" onClick={generate} disabled={loading || !prompt.trim()} className="mt-4 w-full rounded-2xl bg-[var(--primary)] px-5 py-3 text-sm font-black text-white disabled:opacity-50">{loading ? "در حال پردازش..." : `تولید ${mode === "image" ? "تصویر" : mode === "video" ? "ویدیو" : "موسیقی"}`}</button>
    </div>
