...
export default function TextActionsEnhancer() {
  useEffect(() => {
    ...
    const enhance = () => {
      ...
      const shareButton = makeButton("اشتراک‌گذاری متن", "↗", async () => { if (!textarea.value) return; try { if (navigator.share) await navigator.share({ title: "متن استخراج‌شده | توسن", text: textarea.value }); else { await navigator.clipboard.writeText(textarea.value); shareButton.textContent = "✓ متن کپی شد"; window.setTimeout(() => { shareButton.textContent = "↗ اشتراک‌گذاری متن"; }, 1800); } } catch { /* user cancelled */ } });
