...
export type ConditionOperator =
    | "equals" | "not_equals" | "contains" | "not_contains" | "is_true" | "is_false";
...
const fieldTypes: { value: FieldType; label: string }[] = [
    { value: "text", label: "متن کوتاه" }, { value: "textarea", label: "متن بلند" },
    { value: "number", label: "عدد" }, { value: "phone", label: "شماره موبایل" },
    { value: "national_code", label: "کد ملی" }, { value: "email", label: "ایمیل" },
    { value: "password", label: "رمز عبور" }, { value: "date", label: "تاریخ" },
    { value: "select", label: "انتخابی" }, { value: "multiselect", label: "چندانتخابی" },
    { value: "boolean", label: "بله / خیر" }, { value: "checkbox", label: "تیک تأیید" },
];

const operators: { value: ConditionOperator; label: string }[] = [
    { value: "equals", label: "برابر باشد با" },
    { value: "not_equals", label: "برابر نباشد با" },
    { value: "contains", label: "شامل باشد" },
    { value: "not_contains", label: "شامل نباشد" },
    { value: "is_true", label: "فعال / بله باشد" },
    { value: "is_false", label: "غیرفعال / خیر باشد" },
];

function id() { return `${Date.now()}-${Math.random().toString(36).slice(2, 9)}`; }
function cleanName(value: string) {
    return value.trim().replace(/\s+/g, "_").replace(/[^a-zA-Z0-9_\u0600-\u06FF]/g, "_");
}
function uniqueName(raw: string, fields: FormField[], currentId: string) {
    const base = cleanName(raw) || `field_${Date.now()}`;
    if (!fields.some(f => f.id !== currentId && f.name === base)) return base;
    let i = 2;
    while (fields.some(f => f.id !== currentId && f.name === `${base}_${i}`)) i++;
    return `${base}_${i}`;
}
function uniqueOptionValue(raw: string, options: FormOption[], index: number) {
    const base = cleanName(raw) || `option_${index + 1}`;
    if (!options.some((o, i) => i !== index && o.value === base)) return base;
    let n = 2;
    while (options.some((o, i) => i !== index && o.value === `${base}_${n}`)) n++;
    return `${base}_${n}`;
}
function defaultOptions(): FormOption[] { return [{ label: "گزینه اول", value: "option_1" }]; }
function newField(): FormField {
    return { id: id(), type: "text", label: "فیلد جدید", name: `field_${Date.now()}`, placeholder: "", description: "", required: false };
}
...
export default function ServiceFormBuilder({ value, onChange }: Props) {
        ...
        for (const field of next) {
            ...
            if ((field.type === "select" || field.type === "multiselect") && field.options) {
                ...
                for (const option of field.options) {
                    if (!option.value.trim()) { setValidationError(`مقدار فنی یکی از گزینه‌های «${field.label}» خالی است.`); return; }
                    if (values.has(option.value)) { setValidationError(`مقدار فنی گزینه‌های «${field.label}» تکراری است.`); return; }
    ...
    function updateField(fieldId: string, changes: Partial<FormField>) {
        const next = fields.map(f => f.id === fieldId ? { ...f, ...changes } : f);
        commit(next);
    }
    function removeField(fieldId: string) {
        if (!confirm("آیا از حذف این فیلد مطمئن هستید؟")) return;
        const removed = fields.find(f => f.id === fieldId);
        const next = fields.filter(f => f.id !== fieldId).map(f => ({
            ...f,
            conditions: f.conditions?.filter(c => c.field !== removed?.name),
        }));
        commit(next);
    }
    function move(index: number, direction: -1 | 1) {
        ...
        const next = [...fields]; [next[index], next[target]] = [next[target], next[index]]; commit(next);
    }
    function addOption(fieldId: string) {
        const field = fields.find(f => f.id === fieldId); if (!field) return;
        const options = [...(field.options || []), { label: `گزینه ${(field.options?.length || 0) + 1}`, value: uniqueOptionValue(`option_${(field.options?.length || 0) + 1}`, field.options || [], field.options?.length || 0) }];
        updateField(fieldId, { options });
    }
    function updateOption(fieldId: string, index: number, changes: Partial<FormOption>) {
        const field = fields.find(f => f.id === fieldId); if (!field) return;
        const options = [...(field.options || [])];
        options[index] = { ...options[index], ...changes };
        updateField(fieldId, { options });
    }
    function removeOption(fieldId: string, index: number) {
        const field = fields.find(f => f.id === fieldId); if (!field) return;
        const options = [...(field.options || [])]; options.splice(index, 1); updateField(fieldId, { options });
    }
    function addCondition(fieldId: string) {
        const index = fields.findIndex(f => f.id === fieldId); if (index <= 0) return;
        const parent = fields[index - 1];
        const boolean = parent.type === "boolean" || parent.type === "checkbox";
        const condition: FieldCondition = { field: parent.name, operator: boolean ? "is_true" : "equals", value: boolean ? true : parent.options?.[0]?.value || "" };
        updateField(fieldId, { conditions: [...(fields[index].conditions || []), condition] });
    }
    function updateCondition(fieldId: string, index: number, changes: Partial<FieldCondition>) {
        const field = fields.find(f => f.id === fieldId); if (!field) return;
        const conditions = [...(field.conditions || [])]; conditions[index] = { ...conditions[index], ...changes }; updateField(fieldId, { conditions });
    }
    function removeCondition(fieldId: string, index: number) {
        const field = fields.find(f => f.id === fieldId); if (!field) return;
        const conditions = [...(field.conditions || [])]; conditions.splice(index, 1); updateField(fieldId, { conditions: conditions.length ? conditions : undefined });
    }
    ...
    return <div dir="rtl" className="space-y-4">
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            <div><h3 className="text-lg font-bold">فرم اطلاعات مشتری</h3><p className="text-sm text-gray-500 mt-1">فیلدهای موردنیاز مشتری را برای این خدمت تعریف کنید.</p></div>
            <button type="button" onClick={() => commit([...fields, newField()])} className="bg-[#09967C] text-white px-4 py-2 rounded-xl">+ افزودن فیلد</button>
        </div>
        ...
        {fields.map((field, index) => {
            const parents = fields.slice(0, index).filter(f => f.name && f.name !== field.name);
            return <div key={field.id} className="border border-gray-200 rounded-2xl p-5 bg-gray-50 space-y-4">
                <div className="flex items-center justify-between"><div className="flex items-center gap-2"><span className="bg-white border rounded-lg px-3 py-1 text-sm">فیلد {index + 1}</span><button type="button" disabled={!index} onClick={() => move(index, -1)} className="bg-white border rounded-lg px-3 py-1 disabled:opacity-30">↑</button><button type="button" disabled={index === fields.length - 1} onClick={() => move(index, 1)} className="bg-white border rounded-lg px-3 py-1 disabled:opacity-30">↓</button></div><button type="button" onClick={() => removeField(field.id)} className="text-red-600 text-sm">حذف فیلد</button></div>
