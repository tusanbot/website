...
export default function ServicesCatalog({ services, initialCategory = "all", initialSearch = "" }: Props) {
  ...
  return <div className="space-y-5">
    ...
    <GlassPanel className="overflow-hidden rounded-3xl p-3 sm:p-4">
      ...
      {categories.length > 0 && <div className="mt-3 overflow-x-auto pb-1 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
        <div className="flex min-w-max gap-2" role="list" aria-label="دسته‌بندی خدمات">
          <CategoryLink href="/services" active={category === "all"}>همه <span>{services.length.toLocaleString("fa-IR")}</span></CategoryLink>
          {categories.map((item) => <CategoryLink key={item.slug} href={`/services?category=${encodeURIComponent(item.slug)}${search ? `&q=${encodeURIComponent(search)}` : ""}`} active={category === item.slug}>{item.title} <span>{item.count.toLocaleString("fa-IR")}</span></CategoryLink>)}
        </div>
      </div>}
    </GlassPanel>

    <div className="flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
      <div><div className="text-xs font-bold text-[var(--primary)]">کاتالوگ خدمات</div><h2 className="mt-1 text-2xl font-black text-[var(--text)]">{category === "all" ? "همه خدمات" : categories.find((item) => item.slug === category)?.title || "خدمات"}</h2></div>
      <div className="text-sm text-[var(--muted)]">{filtered.length.toLocaleString("fa-IR")} خدمت</div>
    </div>

    {filtered.length === 0 ? <GlassPanel className="rounded-3xl p-10 text-center"><div className="text-4xl">🔎</div><h2 className="mt-4 text-xl font-black">خدمتی پیدا نشد</h2><p className="mt-2 text-sm text-[var(--muted)]">عبارت جستجو یا دسته‌بندی دیگری را امتحان کنید.</p><Link href="/services" className="mt-5 inline-block font-bold text-[var(--primary)]">نمایش همه خدمات</Link></GlassPanel> : <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3">
      {filtered.map((service) => <article key={service.id} className="group flex min-h-[190px] flex-col rounded-2xl border border-[var(--border)] bg-[var(--surface)] p-4 shadow-sm transition duration-200 hover:-translate-y-1 hover:border-[var(--primary)]/35 hover:shadow-md">
        <div className="flex items-center gap-3">
          <div className="flex h-12 w-12 shrink-0 items-center justify-center rounded-xl bg-[var(--primary)]/10 text-2xl transition group-hover:scale-105">{service.icon || "📄"}</div>
          <div className="min-w-0 flex-1"><h3 className="truncate text-base font-black text-[var(--text)]">{service.title}</h3>{service.category && <span className="mt-1 inline-block text-[11px] font-bold text-[var(--muted)]">{service.category}</span>}</div>
        </div>
        <p className="mt-3 line-clamp-2 text-sm leading-6 text-[var(--muted)]">{service.description || "ثبت سفارش آنلاین این خدمت از طریق توسن."}</p>
        <div className="mt-auto flex items-center justify-between gap-2 pt-3"><span className="text-sm font-black text-[var(--primary)]">{service.price > 0 ? `${service.price.toLocaleString("fa-IR")} تومان` : "تماس بگیرید"}</span><PrimaryLinkButton href={`/services/${encodeURIComponent(service.slug)}`}>مشاهده ←</PrimaryLinkButton></div>
      </article>)}
    </div>}
  </div>;
}
