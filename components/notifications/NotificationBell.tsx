...
export default function NotificationBell() {
  ...
  useEffect(() => {
    ...
    const channel = supabase
      .channel("central-notifications")
      .on("postgres_changes", { event: "INSERT", schema: "public", table: "notifications" }, (payload) => {
        const next = payload.new as NotificationItem & { recipient_id?: string };
        if (next.recipient_id !== userId) return;
  ...
  return (
    <div className="relative">
      <button
        ...
        className="relative rounded-xl border border-[var(--border)] px-3 py-2 hover:bg-[var(--background)] transition"
      >
        🔔
        {unread > 0 && (
          <span className="absolute -right-1 -top-1 min-w-5 h-5 rounded-full bg-red-500 text-white text-[11px] font-black flex items-center justify-center px-1">
      ...
      {open && (
        <>
          ...
          <div className="absolute left-0 top-12 z-40 w-[min(22rem,calc(100vw-2rem))] rounded-2xl border border-[var(--border)] bg-[var(--surface)] shadow-xl overflow-hidden">
            ...
            {items.length === 0 ? (
              <div className="p-6 text-center text-sm text-[var(--text-muted)]">اعلان جدیدی ندارید.</div>
            ) : (
              <div className="max-h-96 overflow-y-auto divide-y divide-[var(--border)]">
                {items.map((item) => (
                  <button
                    ...
                    className={`w-full text-right px-4 py-3 hover:bg-[var(--background)] ${!item.read_at ? "bg-[var(--primary)]/5" : ""}`}
                  >
