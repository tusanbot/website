...
export default function OrderFiles({ orderId }: Props) {
  ...
  async function loadFiles() {
    ...
    try {
      ...
      setFiles([]);
      setError("دریافت فایل‌های سفارش ناموفق بود.");
    } finally {
      setLoading(false);
    }
  }
  ...
  async function downloadFile(file: OrderFile) {
    ...
    try {
      ...
      const result = await response.json().catch(() => ({}));
      if (!response.ok || !result?.signedUrl) throw new Error(result?.error || "signed URL دریافت نشد.");
