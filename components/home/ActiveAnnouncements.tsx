...
function remainingParts(target: string | null, now: number) {
  ...
  const text = days > 0
    ? `${days.toLocaleString("fa-IR")} روز و ${hours.toLocaleString("fa-IR")} ساعت`
    : `${hours.toLocaleString("fa-IR")} ساعت و ${minutes.toLocaleString("fa-IR")} دقیقه`;
  const percent = Math.max(4, Math.min(100, (diff / (30 * 86400000)) * 100));
  return { text, percent };
}
      ...
      <div className="mx-auto max-w-7xl px-6 lg:px-8">
        ...
        {loading ? (
          <div className="mt-8 grid gap-5 sm:grid-cols-2 xl:grid-cols-4">
            {Array.from({ length: 4 }).map((_, index) => <div key={index} className="h-[350px] rounded-[2rem] border border-[var(--border)] bg-[var(--surface)] animate-pulse" />)}
          </div>
        ) : items.length === 0 ? (
            ...
            {items.map((item) => {
              ...
              return (
                <motion.article
                  ...
                  className={`group relative h-[350px] cursor-pointer overflow-hidden rounded-[2rem] border border-black/5 shadow-[0_18px_55px_rgba(0,0,0,0.10)] transition-all duration-300 hover:-translate-y-1 hover:shadow-[0_28px_75px_rgba(0,0,0,0.16)] ${tone.strong}`}
                >
                  ...
                  <div className="relative z-10 flex h-full flex-col p-5 text-white">
                    <div className="flex items-start justify-between gap-3">
                      <div className="flex h-14 w-14 shrink-0 items-center justify-center rounded-2xl bg-white/90 text-3xl shadow-lg">
                        {item.services?.icon || (item.type === "registration" ? "📝" : "📢")}
                      </div>
                      <span className="rounded-full bg-white/90 px-3 py-1.5 text-[11px] font-black text-black/75 shadow-sm">
                    ...
                    <div className="mt-auto rounded-2xl bg-black/15 p-3 backdrop-blur-sm">
                      ...
                      <div className="mt-2 h-2 overflow-hidden rounded-full bg-white/30">
                        <motion.div initial={{ width: 0 }} animate={{ width: `${remaining.percent}%` }} transition={{ duration: 0.8 }} className="h-full rounded-full bg-white" />
                      </div>
                      <div className="mt-3 text-center text-[11px] font-bold text-white/75 transition-opacity group-hover:opacity-100">برای مشاهده جزئیات مکث کنید ←</div>
                    </div>
                  </div>
                  ...
                  <motion.div
                    ...
                    className={`absolute inset-0 z-20 flex flex-col rounded-[1.9rem] p-5 backdrop-blur-xl ${tone.soft} bg-opacity-95`}
                  >
                    <div className="flex items-center justify-between gap-3">
                      <span className={`rounded-full bg-white/90 px-3 py-1 text-[11px] font-black shadow-sm ${tone.text}`}>{tone.label}</span>
                    ...
                    {description && <p className="mt-1.5 line-clamp-2 text-xs leading-5 text-[var(--text-muted)]">{description}</p>}
                    ...
                    <div className="mt-2 grid grid-cols-2 gap-2 text-[10px] font-bold text-[var(--text-muted)]">
                      ...
                      {endDate && <div className="rounded-xl bg-white/65 p-2.5 shadow-sm dark:bg-black/10"><div>پایان ثبت‌نام</div><div className="mt-0.5 font-black text-[var(--text)]">{endDate}</div></div>}
                      {startDate && <div className="rounded-xl bg-white/65 p-2.5 shadow-sm dark:bg-black/10"><div>شروع ثبت‌نام</div><div className="mt-0.5 font-black text-[var(--text)]">{startDate}</div></div>}
                      <div className="rounded-xl bg-white/65 p-2.5 shadow-sm dark:bg-black/10"><div>زمان باقی‌مانده</div><div className={`mt-0.5 font-black ${tone.text}`}>{remaining.text}</div></div>
                    </div>

                    {docs.length > 0 && (
                      <div className="mt-2 rounded-xl bg-white/65 p-2.5 shadow-sm dark:bg-black/10">
                        <div className="text-[10px] font-black text-[var(--text-muted)]">مدارک لازم</div>
                        <div className="mt-1 flex flex-wrap gap-1">
                          {docs.map((doc) => <span key={doc} className="rounded-lg bg-white/75 px-2 py-1 text-[9px] font-bold text-[var(--text)] dark:bg-black/10">{doc}</span>)}
                        </div>
                      </div>
                    )}
