...
import { Instagram, Send, Youtube, Play, MessageCircle, ArrowLeft, Sparkles } from "lucide-react";
...
function PlatformIcon({ slug }: { slug: string }) {
    ...
    if (slug === "instagram") return <Instagram {...props} />;
    if (slug === "telegram") return <Send {...props} />;
    if (slug === "youtube" || slug === "aparat") return <Youtube {...props} />;
...
export default function SocialServicesPreview() {
    ...
    const featured = platforms.slice(0, 3);
        ...
        <section className="relative py-20 sm:py-24">
            <div className="mx-auto max-w-7xl px-6 lg:px-8">
                <SectionHeader
                    ...
                    description="خدمات افزایش تعامل و رشد شبکه‌های اجتماعی را از توسن، آنلاین و با امکان پیگیری سفارش دریافت کنید."
                    align="center"
                />
                ...
                {loading ? (
                    <div className="mt-10 grid gap-6 md:grid-cols-3">
                        {Array.from({ length: 3 }).map((_, index) => (
                            <div key={index} className="h-64 rounded-3xl border border-[var(--border)] bg-[var(--surface)] animate-pulse" />
                        ))}
                    </div>
                ) : featured.length > 0 ? (
                    <div className="mt-10 grid gap-6 md:grid-cols-3">
                        {featured.map((platform, index) => {
                            ...
                            const count = services.filter((service) => service.platform_id === platform.id).length;
                            return (
                                <motion.div
                                    key={platform.id}
                                ...
                                >
                                    <Link href={`/social?platform=${encodeURIComponent(platform.slug)}`} className="block h-full">
                                        <GlassPanel className={`group relative h-full min-h-64 overflow-hidden rounded-3xl border ${theme.border} bg-[var(--surface)] p-6 transition duration-300 hover:-translate-y-2 hover:shadow-[0_22px_70px_rgba(9,150,124,0.16)]`}>
                                            ...
                                            <div className={`flex h-16 w-16 items-center justify-center rounded-2xl ${theme.soft} ${theme.icon}`}>
                                                <PlatformIcon slug={platform.slug} />
                                            </div>
                                            <div className="mt-6">
                                                <div className="flex items-center gap-2 text-xs font-bold text-[var(--primary)]">
                                                    <Sparkles size={14} /> خدمات شبکه اجتماعی
                                                </div>
                                                <h3 className="mt-2 text-2xl font-black text-[var(--text)]">{platform.name}</h3>
                                                <p className="mt-3 leading-7 text-[var(--text-muted)]">
                                                    {count > 0 ? `${count.toLocaleString("fa-IR")} سرویس فعال برای این پلتفرم` : "مشاهده سرویس‌های فعال این پلتفرم"}
                                                </p>
                                            </div>
                                            <div className="mt-6 flex items-center justify-between rounded-2xl border border-[var(--border)] bg-[var(--surface-secondary)] px-4 py-3">
                                                <span className="text-sm font-black text-[var(--text)]">مشاهده خدمات</span>
                                                <ArrowLeft size={17} className="text-[var(--primary)] transition-transform group-hover:-translate-x-1" />
                                            </div>
                                        </GlassPanel>
                                    </Link>
                                </motion.div>
                    ...
                    <div className="mt-10 rounded-3xl border border-dashed border-[var(--border)] bg-[var(--surface)] p-8 text-center text-[var(--text-muted)]">
                        خدمات شبکه‌های اجتماعی به‌زودی در این بخش نمایش داده می‌شود.
                    </div>
                )}

                <div className="mt-8 flex justify-center">
                    <Link href="/social" className="inline-flex items-center gap-2 rounded-2xl bg-[var(--primary)] px-6 py-3.5 font-black text-white transition hover:opacity-90">
                        مشاهده همه خدمات شبکه‌های اجتماعی <ArrowLeft size={18} />
                    </Link>
                </div>
            </div>
        </section>
