...
export default function ToolsPreview() {
    ...
    return (
        <section className="relative py-16 md:py-20" dir="rtl">
                ...
                <GlassPanel className="overflow-hidden rounded-[2rem] border border-[var(--border)] bg-[var(--surface)]/90 p-6 sm:p-8">
                    <div className="flex flex-col gap-5 md:flex-row md:items-end md:justify-between">
                        ...
                        <Link href="/tools" className="shrink-0">
                            <TusanButton variant="secondary">مشاهده همه ابزارها ←</TusanButton>
                        </Link>
                    </div>
                            ...
                            <Link key={tool.id} href={tool.enabled && tool.href ? tool.href : "/tools"} className="group rounded-2xl border border-[var(--border)] bg-[var(--surface-secondary)] p-4 transition hover:-translate-y-1 hover:border-[var(--primary)]/30">
                                <div className="flex items-center gap-3">
                                    ...
                                    <div className="min-w-0">
                                        <h3 className="truncate font-black text-[var(--text)]">{tool.title}</h3>
                                        <p className="mt-1 text-xs text-[var(--text-muted)]">{tool.type === "ai" ? "هوش مصنوعی" : "ابزار کاربردی"}</p>
                                    </div>
                                </div>
                            </Link>
                        ))}
