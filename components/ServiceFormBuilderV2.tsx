...
function FieldEditor({ field, allFields, onChange, nested = false }: { field: FormField; allFields: FormField[]; onChange: (field: FormField) => void; nested?: boolean }) {
  ...
  const addCondition = () => { const parents = allFields.filter(f => f.id !== field.id && f.name); const parent = parents[parents.length - 1]; if (!parent) return; const bool = parent.type === "boolean" || parent.type === "checkbox"; update({ conditions: [...(field.conditions || []), { field: parent.name, fieldId: parent.id, operator: bool ? "is_true" : "equals", value: bool ? true : parent.options?.[0]?.value || "" }] }); };
  ...
  const removeCondition = (i: number) => { const conditions = (field.conditions || []).filter((_, n) => n !== i); update({ conditions: conditions.length ? conditions : undefined }); };
  ...
  return <div className="space-y-4">
    ...
    {!nested && allFields.length > 1 && <div className="border-t pt-4"><div className="flex items-center justify-between mb-3"><div><h4 className="font-bold text-[#09967C]">نمایش شرطی</h4><p className="text-xs text-gray-500">این فیلد می‌تواند بر اساس فیلدهای قبلی نمایش داده شود.</p></div><button type="button" onClick={addCondition} className="text-[#09967C] text-sm font-bold">+ افزودن شرط</button></div>{(field.conditions || []).map((c, i) => <div key={`${field.id}-condition-${i}`} className="grid md:grid-cols-[1fr_1fr_1fr_auto] gap-2 mb-2"><select value={c.fieldId || c.field} onChange={e => { const p = allFields.find(x => x.id === e.target.value); updateCondition(i, { fieldId: p?.id, field: p?.name || e.target.value, operator: p?.type === "boolean" || p?.type === "checkbox" ? "is_true" : "equals", value: p?.type === "boolean" || p?.type === "checkbox" ? true : p?.options?.[0]?.value || "" }); }} className="border rounded-lg px-3 py-2 bg-white">{allFields.filter(f => f.id !== field.id).map(f => <option key={f.id} value={f.id}>{f.label}</option>)}</select><select value={c.operator} onChange={e => updateCondition(i, { operator: e.target.value as ConditionOperator })} className="border rounded-lg px-3 py-2 bg-white">{operators.map(o => <option key={o.value} value={o.value}>{o.label}</option>)}</select><input value={typeof c.value === "boolean" ? (c.value ? "true" : "false") : String(c.value ?? "")} onChange={e => updateCondition(i, { value: e.target.value })} className="border rounded-lg px-3 py-2 bg-white" dir="ltr" /><button type="button" onClick={() => removeCondition(i)} className="text-red-500">حذف</button></div>)}</div>}
