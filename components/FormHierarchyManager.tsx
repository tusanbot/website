...
export default function FormHierarchyManager({ serviceId }: Props) {
    ...
    async function loadForms() {
        ...
        const root = rows.find((row) => row.form_type === "parent" && !row.parent_form_id) || null;
        setParent(root);
    ...
    async function saveChild(child: FormRow) {
        ...
        setSaving(true);
        setError("");
        const { error: updateError } = await supabase.from("custom_forms").update({
    ...
    if (loading) {
        return <GlassPanel className="p-5"><p className="text-sm text-gray-500">در حال دریافت ساختار فرم‌ها...</p></GlassPanel>;
    }
    ...
    return (
        <GlassPanel className="p-5 space-y-5">
            <div>
                ...
                <p className="text-sm text-gray-500 mt-1">هر خدمت می‌تواند یک فرم مادر داشته باشد و فرم‌های عادی آن به‌عنوان فرزند زیرمجموعه فرم مادر قرار بگیرند.</p>
            </div>
                ...
                <>
                    ...
                    <div className="space-y-3">
                        <div className="flex items-center justify-between">
                            <div><h4 className="font-bold">فرم‌های فرزند</h4><p className="text-sm text-gray-500">مثلاً سایپا، ایران‌خودرو، کرمان‌موتور و...</p></div>
                            <TusanButton type="button" onClick={() => setShowNewChild((value) => !value)}>{showNewChild ? "بستن" : "+ افزودن فرم فرزند"}</TusanButton>
                        </div>

                        {showNewChild && <TusanCard className="p-5 space-y-5 border-2 border-dashed border-[#09967C]/30">
                        ...
                        {children.length === 0 && !showNewChild && <div className="border border-dashed rounded-xl p-6 text-center text-gray-500">هنوز فرم فرزندی ایجاد نشده است.</div>}
