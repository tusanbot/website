...
export default function ContentStudioMediaPanel({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    const sync = () => {
      ...
      setCurrent(s.index);
      setSlots(prev => Array.from({ length: Math.max(s.count, prev.length) }, (_, i) => prev[i] ?? { id: uid(), sourceIndex: Math.min(i, s.count - 1), enabled: true }));
    };
    sync();
  ...
  useEffect(() => {
    if (audioDuration > 0 && segments.length !== slots.length) {
      setSegments(equalSegments(slots.length, audioDuration));
      setSelected(Math.min(selected, Math.max(0, slots.length - 1)));
    }
  }, [audioDuration, slots.length, segments.length, selected]);
        ...
        {audioDuration > 0 ? <>
          <div ref={timelineRef} className="relative mt-4 h-20 select-none overflow-visible rounded-xl border bg-slate-100" onClick={e => { const r = e.currentTarget.getBoundingClientRect(); seekTo(Math.max(0, Math.min(audioDuration, ((e.clientX - r.left) / r.width) * audioDuration))); }}>
            {segments.map((seg, i) => {
              ...
              return <button key={i} onClick={e => { e.stopPropagation(); setSelected(i); seekTo(seg.start); }} className={`absolute inset-y-0 ${colors[i % colors.length]} ${i === selected ? "ring-4 ring-white/70" : "opacity-80"}`} style={{ left: `${left}%`, width: `${width}%` }}><span className="text-xs font-bold text-white drop-shadow">اسلاید {i + 1}</span></button>;
            })}
