...
export default function ContentStudioMediaPanel({ children }: { children: ReactNode }) {
  ...
  function pointerTime(clientX: number) { const rect = timelineRef.current?.getBoundingClientRect(); if (!rect || !audioDuration) return 0; return Math.max(0, Math.min(visibleDuration || audioDuration, ((clientX - rect.left) / rect.width) * (visibleDuration || audioDuration))); }
