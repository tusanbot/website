...
export default function ContentStudioMediaPanel({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    readLatestStudioAudio().then(item => {
      if (!item) return;
      ...
      audio.onloadedmetadata = () => {
        setAudios(prev => ({ ...prev, [current]: { file, duration: audio.duration, name: file.name } }));
  ...
  async function attachAudio(index: number, file?: File) {
    if (!file) return;
    const audio = document.createElement("audio");
    audio.preload = "metadata";
    audio.src = URL.createObjectURL(file);
  ...
  async function exportVideo() {
    ...
    try {
      ...
      for (let i = 0; i < slideCount; i++) {
        ...
        setProgress(Math.round(((i + 1) / slideCount) * 55));
      }
      if (!captures.length) throw new Error("حداقل یک اسلاید فعال لازم است.");
      setMessage("در حال ساخت ویدیو MP4 با صوت و زمان‌بندی اسلایدها...");
      const output = await encodeSlideSequence(captures, setProgress);
      const url = URL.createObjectURL(output);
      const a = document.createElement("a");
      a.href = url; a.download = `tusan-reel-${Date.now()}.mp4`; a.click();
  ...
  return <div className="space-y-4">
    ...
    <section className="rounded-2xl border border-emerald-200 bg-emerald-50/70 p-4" dir="rtl">
      ...
      <div className="mt-3 flex items-center gap-2 rounded-xl border bg-white px-3 py-2 text-xs"><Mic2 className="h-4 w-4 text-emerald-600" /> برای اتصال مستقیم به «تبدیل متن به صوت»، فایل تولیدشده در ابزار TTS را با گزینه «ارسال به استودیو» وارد کنید.</div>
    </section>
  </div>;
}
