...
import AdminOrderStatus, {
    type OrderStatusStats,
} from "@/components/admin/AdminOrderStatus";
import AdminTopServices, {
    type ServiceStat,
} from "@/components/admin/AdminTopServices";
...
export default function AdminDashboard() {
    ...
    const [stats, setStats] =
        useState<Stats>(DEFAULT_STATS);

    const [orderStatuses, setOrderStatuses] =
        useState<OrderStatusStats>(
    ...
    async function loadDashboard() {
        ...
        try {
            ...
            const completedOrders =
                completedRevenueRes.data?.length || 0;
            ...
            ordersStatusRes.data?.forEach((order) => {
                const status =
                    order.status as keyof OrderStatusStats;
            ...
            const formattedRecentOrders: RecentOrder[] =
                (recentOrdersRes.data || []).map(
                    (order: any) => ({
                        id: order.id,
                        tracking_code:
                            order.tracking_code,
            ...
            setStats({
                totalOrders:
                    totalOrdersRes.count || 0,

                todayOrders:
                    todayOrdersRes.count || 0,

                processingOrders:
                    processingOrdersRes.count || 0,
                    ...
                    <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                        <div>
                            ...
                            <p className="text-[var(--text-muted)] mt-1">
                                مشاهده، جستجو، فیلتر و مدیریت سفارش‌های مشتریان
                            </p>
                        </div>

                        <Link href="/admin/orders">
                            <TusanButton>
                                ورود به مدیریت سفارش‌ها
                            </TusanButton>
                        </Link>
                    </div>
                </GlassPanel>
                ...
                <GlassPanel className="p-8">
                    <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                        <div>
                            ...
                            <p className="text-[var(--text-muted)] mt-1">
                                مدیریت خدمات و فرم‌های قابل ارائه
                            </p>
                        </div>

                        <Link href="/admin/services">
                            <TusanButton>
                                ورود به مدیریت خدمات
                            </TusanButton>
                        </Link>
                    </div>
                </GlassPanel>
    ...
    function getPageDescription() {
        switch (activeTab) {
            case "dashboard":
                return "نمای کلی عملکرد کافی‌نت";

            case "orders":
                return "مشاهده و مدیریت سفارش‌های مشتریان";

            case "services":
                return "مدیریت خدمات و فرم‌های قابل ارائه";

            case "users":
                return "مدیریت کاربران ثبت‌نام‌شده";

            case "messages":
                return "مدیریت پیام‌های مشتریان";

            case "reports":
                return "گزارش‌های مالی و عملکردی";

            case "settings":
                return "تنظیمات پنل مدیریت";
