...
type UserProfile = {
    ...
    profile_completed: boolean; staff_roles: StaffAssignment[]; tags: { id: string; name: string; slug: string; color: string | null; expires_at: string | null }[];
};
...
export default function AdminUsers() {
    ...
    const filteredUsers = useMemo(() => {
        ...
        return users.filter(user => {
            ...
            return [user.full_name, user.phone, user.national_code, user.email, ...user.staff_roles.map(r => r.name), ...user.tags.map(t => t.name)].filter(Boolean).some(value => String(value).toLowerCase().includes(query));
        });
    ...
    return <div className="space-y-6">
        ...
        {loading ? <GlassPanel className="p-10 text-center text-gray-500">در حال دریافت کاربران...</GlassPanel> : error ? <GlassPanel className="bg-red-50 border border-red-200 text-red-700 rounded-2xl p-6 text-center">{error}</GlassPanel> : filteredUsers.length === 0 ? <GlassPanel className="p-10 text-center text-gray-500">کاربری با این مشخصات پیدا نشد.</GlassPanel> : <TusanTable columns={[{ key: "user", title: "کاربر" },{ key: "phone", title: "موبایل" },{ key: "role", title: "نقش حساب", align: "center" },{ key: "staff", title: "مقام کارکنان", align: "center" },{ key: "profile", title: "پروفایل", align: "center" },{ key: "orders", title: "سفارش‌ها", align: "center" },{ key: "created", title: "تاریخ عضویت" },{ key: "actions", title: "عملیات", align: "left" }]} rows={filteredUsers.map(user => ({
            user: <div className="flex items-center gap-3"><div className="w-10 h-10 rounded-full bg-[var(--primary)]/10 text-[var(--primary)] flex items-center justify-center font-bold">{(user.full_name || user.email || "؟").trim().charAt(0)}</div><div><div className="font-bold text-[var(--text)]">{user.full_name || "بدون نام"}</div><div className="text-xs text-[var(--text-muted)] mt-1">{user.email || `${user.id.slice(0, 8)}...`}</div>{user.tags.length > 0 && <div className="mt-1 flex flex-wrap gap-1">{user.tags.map(tag => <span key={tag.id} className="rounded-full px-2 py-0.5 text-[10px] font-black border" style={{ borderColor: tag.color || "var(--primary)", color: tag.color || "var(--primary)" }}>{tag.name}</span>)}</div>}</div></div>,
