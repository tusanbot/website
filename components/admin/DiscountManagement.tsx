...
export default function DiscountManagement(){
 ...
 return <div dir="rtl" className="space-y-6">
  ...
  <GlassPanel className="p-5 space-y-4">
   ...
   <div className="grid md:grid-cols-3 gap-3"><select value={codeForm.discount_id} onChange={e=>setCodeForm(v=>({...v,discount_id:e.target.value}))} className="rounded-xl border border-[var(--border)] px-3 py-2 bg-white">{discounts.map(d=><option key={d.id} value={d.id}>{d.name}</option>)}</select><TusanInput placeholder="کد، مثلاً TOSAN10" value={codeForm.code} onChange={e=>setCodeForm(v=>({...v,code:e.target.value}))}/><TusanInput placeholder="سقف کل مصرف" type="number" value={codeForm.usage_limit} onChange={e=>setCodeForm(v=>({...v,usage_limit:e.target.value}))}/><TusanInput placeholder="سقف هر کاربر" type="number" value={codeForm.per_user_limit} onChange={e=>setCodeForm(v=>({...v,per_user_limit:e.target.value}))}/><TusanInput placeholder="شروع ISO" value={codeForm.starts_at} onChange={e=>setCodeForm(v=>({...v,starts_at:e.target.value}))}/><TusanInput placeholder="پایان ISO" value={codeForm.ends_at} onChange={e=>setCodeForm(v=>({...v,ends_at:e.target.value}))}/><TusanButton onClick={addCode}>افزودن کد</TusanButton></div>
  </GlassPanel>
  <GlassPanel className="p-5 space-y-4">
