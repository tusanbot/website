...
export default function BlogInteractions({ postId, postTitle, postUrl }: Props) {
  ...
  const load = async () => {
    ...
    if (visitorId) {
      await supabase.rpc("blog_post_record_view", { p_post_id: postId, p_visitor_id: visitorId });
    }
  ...
  const react = async (reaction: "like" | "dislike") => {
    ...
    try {
      ...
      setMessage("ارتباط با سرور برای ثبت واکنش انجام نشد.");
    } finally {
      setLoading(false);
    }
  };
  ...
  const rate = async (value: number) => {
    ...
    try {
      ...
      setMessage("ارتباط با سرور برای ثبت امتیاز انجام نشد.");
    } finally {
      setLoading(false);
    }
  };
  ...
  const submitComment = async () => {
    ...
    try {
      ...
      else { setComment(""); setMessage("نظر شما ثبت شد و پس از تأیید مدیر نمایش داده خواهد شد."); }
      await load();
    } catch {
      setMessage("ارتباط با سرور برای ثبت نظر انجام نشد.");
    } finally {
      setLoading(false);
    }
  };
