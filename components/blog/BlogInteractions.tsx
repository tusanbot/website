...
export default function BlogInteractions({ postId, postTitle, postUrl }: Props) {
  ...
  const load = async () => {
    ...
    if (!statsResult.error && statsResult.data) {
      ...
      setEngagement({ likes: Number(stats.likes ?? 0), dislikes: Number(stats.dislikes ?? 0), rating_count: Number(stats.rating_count ?? 0), rating_average: Number(stats.rating_average ?? 0), comment_count: Number(stats.comment_count ?? 0), view_count: Number(stats.view_count ?? 0) });
    }
    ...
    } else { setMyReaction(null); setMyRating(null); }
  };
  ...
  const react = async (reaction: "like" | "dislike") => {
    ...
    const result = myReaction === reaction ? await supabase.from("blog_reactions").delete().eq("post_id", postId).eq("user_id", userId) : await supabase.from("blog_reactions").upsert({ post_id: postId, user_id: userId, reaction }, { onConflict: "post_id,user_id" });
    if (result.error) setMessage("ثبت واکنش انجام نشد. لطفاً دوباره تلاش کنید."); else setMyReaction(myReaction === reaction ? null : reaction);
  ...
  const rate = async (value: number) => {
    ...
    if (result.error) setMessage("ثبت امتیاز انجام نشد. لطفاً دوباره تلاش کنید."); else setMyRating(value);
    await load(); setLoading(false);
  };
  const submitComment = async () => {
    const text = comment.trim(); if (text.length < 2) return setMessage("متن نظر باید حداقل ۲ کاراکتر باشد.");
    ...
    const { error } = await supabase.from("blog_comments").insert({ post_id: postId, user_id: user.id, author_name: name, content: text, status: "pending" });
    if (error) setMessage("ثبت نظر انجام نشد. لطفاً دوباره تلاش کنید."); else { setComment(""); setMessage("نظر شما ثبت شد و تا زمان تأیید، با برچسب «در انتظار تأیید» نمایش داده می‌شود."); }
    await load(); setLoading(false);
  };
  const share = async (target: string) => { try { if (target === "copy") { await navigator.clipboard?.writeText(postUrl); setMessage("لینک مقاله کپی شد."); return; } if (target === "native" && navigator.share) { await navigator.share({ title: postTitle, url: postUrl }); return; } if (target === "native") { await navigator.clipboard?.writeText(postUrl); setMessage("لینک مقاله کپی شد."); return; } window.open(target, "_blank", "noopener,noreferrer,width=720,height=640"); } catch { setMessage("اشتراک‌گذاری لغو شد یا در این مرورگر در دسترس نیست."); } };
