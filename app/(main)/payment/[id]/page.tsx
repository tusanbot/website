...
export default function PaymentPage() {
    ...
    async function loadOrder() {
        ...
        const { data, error: orderError } = await supabase.from("orders").select("id,tracking_code,status,price,created_at,services(title,icon)").eq("id", orderId).eq("user_id", user.id).single();
    ...
    async function continuePayment() {
        ...
        try {
            ...
            if (method === "card_to_card") {
                const response = await fetch("/api/payments/card-to-card/create", { method: "POST", headers: { "Content-Type": "application/json", Authorization: `Bearer ${session.access_token}` }, body: JSON.stringify({ orderId }) });
            ...
            const response = await fetch("/api/payments/zarinpal/create", { method: "POST", headers: { "Content-Type": "application/json", Authorization: `Bearer ${session.access_token}` }, body: JSON.stringify({ orderId }) });
            const result = await response.json().catch(() => ({}));
    ...
    return <div dir="rtl" className="min-h-screen page-background p-6 text-[var(--text)]"><div className="max-w-2xl mx-auto space-y-5">
        ...
        <div className="grid gap-4"><button type="button" onClick={() => setMethod("online")} className={`text-right rounded-2xl border-2 p-5 transition ${method === "online" ? "border-[#09967C] bg-[#09967C]/5" : "border-gray-200 bg-white"}`}><div className="font-bold text-lg">💳 پرداخت آنلاین</div><div className="text-sm text-gray-500 mt-2">پرداخت امن از طریق درگاه زرین‌پال</div></button><button type="button" onClick={() => setMethod("card_to_card")} className={`text-right rounded-2xl border-2 p-5 transition ${method === "card_to_card" ? "border-[#09967C] bg-[#09967C]/5" : "border-gray-200 bg-white"}`}><div className="font-bold text-lg">🏦 کارت به کارت</div><div className="text-sm text-gray-500 mt-2">واریز مبلغ و ارسال رسید برای بررسی مدیریت</div></button></div>
        {method === "card_to_card" && <TusanCard className="p-6 border border-amber-200 bg-amber-50"><h2 className="font-bold text-lg mb-4">اطلاعات واریز</h2><div className="space-y-3"><div><div className="text-sm text-gray-500">شماره کارت</div><div className="font-bold mt-1">{cardNumber}</div></div><div><div className="text-sm text-gray-500">به نام</div><div className="font-bold mt-1">{accountOwner}</div></div><div><div className="text-sm text-gray-500">مبلغ صورتحساب</div><div className="font-bold mt-1">{amount.toLocaleString("fa-IR")} تومان</div></div></div><div className="mt-5 rounded-xl bg-white border border-amber-200 p-4 leading-7 text-sm">برای نهایی شدن سفارش، مبلغ صورتحساب را حداکثر تا <strong>۲۴ ساعت آینده</strong> واریز کنید. پس از واریز، رسید پرداخت را از بخش «سفارش‌های من» برای همین سفارش آپلود نمایید.</div></TusanCard>}
