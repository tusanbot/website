...
export default function PaymentPage() {
    ...
    async function continuePayment() {
        ...
        try {
            ...
            if (!response.ok || !result?.url) throw new Error(result?.error || "ایجاد پرداخت آنلاین ناموفق بود.");
            window.location.href = result.url;
        } catch (err: any) {
            console.error(err);
            setError(err?.message || "خطایی هنگام ایجاد پرداخت رخ داد.");
    ...
    const cardNumber = "شماره کارت را در تنظیمات پرداخت وارد کنید";
    const accountOwner = "نام صاحب حساب را در تنظیمات پرداخت وارد کنید";
