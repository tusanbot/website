...
function money(value: number) { return new Intl.NumberFormat("fa-IR").format(Math.max(0, Math.round(value))); }

export default function InvoiceToolPage() {
  const [seller, setSeller] = useState("کافی‌نت توسن");
  const [customer, setCustomer] = useState(""); const [phone, setPhone] = useState(""); const [invoiceNo, setInvoiceNo] = useState("");
  const [date, setDate] = useState(today); const [notes, setNotes] = useState("");
  const [logo, setLogo] = useState<string | null>(null); const [seal, setSeal] = useState<string | null>(null);
  const logoInputRef = useRef<HTMLInputElement>(null); const sealInputRef = useRef<HTMLInputElement>(null); const invoiceRef = useRef<HTMLDivElement>(null);
  const [items, setItems] = useState<Item[]>([{ id: 1, title: "", qty: 1, price: 0, discount: 0 }]);
  const totals = useMemo(() => { const subtotal = items.reduce((s, i) => s + i.qty * i.price, 0); const discount = items.reduce((s, i) => s + Math.min(i.qty * i.price, i.discount), 0); return { subtotal, discount, total: Math.max(0, subtotal - discount) }; }, [items]);
  ...
  const readImage = (file: File, setter: (value: string | null) => void) => { if (!file.type.startsWith("image/")) return; const reader = new FileReader(); reader.onload = () => setter(typeof reader.result === "string" ? reader.result : null); reader.readAsDataURL(file); };
  ...
  const exportPdf = async () => {
    ...
    try {
      ...
      while (offset < imageHeight - 0.1) {
        if (page > 0) pdf.addPage(); const sliceHeightMm = Math.min(usableHeight, imageHeight - offset); const sourceY = Math.floor((offset / imageHeight) * canvas.height); const sourceHeight = Math.max(1, Math.min(canvas.height - sourceY, Math.ceil((sliceHeightMm / imageHeight) * canvas.height)));
        const slice = document.createElement("canvas"); slice.width = canvas.width; slice.height = sourceHeight; const context = slice.getContext("2d"); if (!context) throw new Error("Canvas context unavailable");
        context.fillStyle = "#fff"; context.fillRect(0, 0, slice.width, slice.height); context.drawImage(canvas, 0, sourceY, canvas.width, sourceHeight, 0, 0, canvas.width, sourceHeight);
        const actualHeight = (sourceHeight * imageWidth) / canvas.width; pdf.addImage(slice.toDataURL("image/jpeg", 0.92), "JPEG", margin, margin, imageWidth, Math.min(actualHeight, usableHeight), undefined, "FAST"); offset += sliceHeightMm; page += 1;
      }
      pdf.save(`invoice-${invoiceNo || "tusan"}.pdf`);
    } catch (error) { console.error("Invoice PDF export failed", error); window.alert("ساخت فایل PDF انجام نشد. لطفاً دوباره تلاش کنید."); }
  };
  return (
    <main dir="rtl" className="min-h-screen page-background p-4 sm:p-6"><div className="mx-auto max-w-6xl">
      <div className="no-print mb-6 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between"><div><h1 className="text-2xl font-black">فاکتورساز</h1><p className="mt-1 text-sm text-[var(--text-muted)]">ساخت فاکتور فارسی، چاپ و خروجی PDF</p></div><div className="flex flex-wrap gap-2"><button type="button" onClick={printInvoice} className="inline-flex items-center gap-2 rounded-xl border border-[var(--border)] bg-[var(--surface)] px-4 py-2 font-bold"><Printer size={18}/> چاپ</button><button type="button" onClick={exportPdf} className="inline-flex items-center gap-2 rounded-xl bg-[var(--primary)] px-4 py-2 font-bold text-white"><FileDown size={18}/> خروجی PDF</button></div></div>
      <div className="no-print mb-6 rounded-3xl border border-[var(--border)] bg-[var(--surface)] p-5 shadow-sm"><div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <label className="text-sm font-bold">صادرکننده<input value={seller} onChange={(e)=>setSeller(e.target.value)} className="mt-2 w-full rounded-xl border border-[var(--border)] bg-transparent px-3 py-2 font-normal"/></label><label className="text-sm font-bold">نام مشتری<input value={customer} onChange={(e)=>setCustomer(e.target.value)} className="mt-2 w-full rounded-xl border border-[var(--border)] bg-transparent px-3 py-2 font-normal"/></label><label className="text-sm font-bold">شماره تماس<input value={phone} onChange={(e)=>setPhone(e.target.value)} className="mt-2 w-full rounded-xl border border-[var(--border)] bg-transparent px-3 py-2 font-normal"/></label><label className="text-sm font-bold">شماره فاکتور<input value={invoiceNo} onChange={(e)=>setInvoiceNo(e.target.value)} className="mt-2 w-full rounded-xl border border-[var(--border)] bg-transparent px-3 py-2 font-normal"/></label><label className="text-sm font-bold">تاریخ<input value={date} onChange={(e)=>setDate(e.target.value)} className="mt-2 w-full rounded-xl border border-[var(--border)] bg-transparent px-3 py-2 font-normal"/></label>
      ...
      <div ref={invoiceRef} className="invoice-paper mx-auto bg-white text-slate-900 shadow-xl print:shadow-none"><div className="p-6 sm:p-10">
        <div className="flex flex-col gap-5 border-b-2 border-slate-900 pb-6 sm:flex-row sm:items-start sm:justify-between"><div className="flex items-start gap-4">{logo&&<img src={logo} alt="لوگو" className="h-16 w-16 object-contain" crossOrigin="anonymous"/>}<div><h2 className="text-3xl font-black">فاکتور</h2><p className="mt-2 text-lg font-bold">{seller||"—"}</p></div></div><div className="text-sm leading-8"><div>شماره: {invoiceNo||"—"}</div><div>تاریخ: {date||"—"}</div></div></div>
