...
const themes: { id: InvoiceTheme; label: string; description: string; previewClass: string }[] = [
  { id: "classic", label: "کلاسیک", description: "رسمی و متعادل", previewClass: "bg-white border-slate-400" },
  { id: "modern", label: "مدرن", description: "تمیز و امروزی", previewClass: "bg-white border-slate-900" },
  { id: "tusan", label: "توسن", description: "هماهنگ با هویت توسن", previewClass: "bg-white border-[#09967c]" },
  { id: "minimal", label: "مینیمال", description: "ساده و کم‌جوهر", previewClass: "bg-white border-slate-300" },
  { id: "formal", label: "رسمی", description: "مناسب شرکت‌ها و سازمان‌ها", previewClass: "bg-slate-50 border-slate-700" },
];
...
export default function InvoiceToolPage() {
  ...
  const exportPdf = async () => {
    ...
    try {
      ...
      const canvas = await html2canvas(invoiceRef.current, { scale: Math.min(2, window.devicePixelRatio || 1.5), backgroundColor: "#ffffff", useCORS: true, allowTaint: false, logging: false });
      const pdf = new jsPDF({ orientation: "portrait", unit: "mm", format: "a4", compress: true });
      const margin = 8, pageWidth = 210, pageHeight = 297, imageWidth = pageWidth - margin * 2, imageHeight = (canvas.height * imageWidth) / canvas.width, usableHeight = pageHeight - margin * 2;
      let offset = 0, page = 0;
      while (offset < imageHeight - 0.1) {
        if (page > 0) pdf.addPage();
        const sliceHeightMm = Math.min(usableHeight, imageHeight - offset), sourceY = Math.floor((offset / imageHeight) * canvas.height), sourceHeight = Math.max(1, Math.min(canvas.height - sourceY, Math.ceil((sliceHeightMm / imageHeight) * canvas.height)));
        const slice = document.createElement("canvas"); slice.width = canvas.width; slice.height = sourceHeight;
        const context = slice.getContext("2d"); if (!context) throw new Error("Canvas context unavailable");
        ...
        offset += sliceHeightMm; page += 1;
      }
      pdf.save(`invoice-${invoiceNo || "tusan"}.pdf`);
    } catch (error) {
      console.error("Invoice PDF export failed", error);
      window.alert("ساخت فایل PDF انجام نشد. لطفاً از گزینه چاپ و سپس Print to PDF استفاده کنید.");
    }
  };

  const themeStyles: Record<InvoiceTheme, { paper: string; header: string; title: string; info: string; tableHead: string; total: string; footer: string }> = {
    classic: { paper: "border border-slate-300", header: "border-b-2 border-slate-800", title: "text-slate-900", info: "bg-slate-50 border border-slate-200", tableHead: "bg-slate-100", total: "border-slate-800", footer: "border-slate-300 text-slate-500" },
    modern: { paper: "border border-slate-200", header: "border-b-4 border-slate-900", title: "text-slate-950", info: "bg-slate-100 rounded-3xl", tableHead: "bg-slate-900 text-white", total: "border-slate-900", footer: "border-slate-300 text-slate-500" },
    tusan: { paper: "border border-[#09967c]/30", header: "border-b-4 border-[#09967c]", title: "text-[#087a66]", info: "bg-[#09967c]/5 border border-[#09967c]/20", tableHead: "bg-[#09967c] text-white", total: "border-[#09967c]", footer: "border-[#09967c]/30 text-[#087a66]" },
    minimal: { paper: "border border-slate-200", header: "border-b border-slate-300", title: "text-slate-800", info: "border-b border-slate-200", tableHead: "bg-white", total: "border-slate-500", footer: "border-slate-200 text-slate-400" },
    formal: { paper: "border-2 border-slate-700", header: "border-b-2 border-slate-700", title: "text-slate-900", info: "bg-slate-100 border border-slate-300", tableHead: "bg-slate-700 text-white", total: "border-slate-700", footer: "border-slate-400 text-slate-600" },
  };
      ...
      <div className="mx-auto max-w-6xl">
        ...
        <div className="no-print mb-6 rounded-3xl border border-[var(--border)] bg-[var(--surface)] p-5 shadow-sm">
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
            ...
            <div className="text-sm font-bold">لوگو و مهر<div className="mt-2 flex flex-wrap gap-2">
        ...
        <div ref={invoiceRef} className={`invoice-paper mx-auto bg-white text-slate-900 shadow-xl print:shadow-none ${current.paper}`}>
          <div className="p-6 sm:p-10">
            ...
            <footer className={`invoice-footer mt-12 border-t pt-4 text-center text-xs ${current.footer}`}>فاکتور ساز رایگان توسن · <span dir="ltr">{SITE_URL}</span></footer>
          </div>
        </div>
      </div>
