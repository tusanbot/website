...
export default function OcrToolPage() {
    ...
    const runOcr = useCallback(async (file: File) => {
        ...
        if (!file.type.startsWith("image/") && file.type !== "application/pdf") {
            setStatus("error");
            setError("فقط فایل‌های تصویری و PDF قابل پردازش هستند.");
            return;
        }
            ...
            const worker = await createWorker("fas+eng", 1, {
                logger: (message) => {
                    if (message.status === "recognizing text" && typeof message.progress === "number") {
                        ...
                        setProgress(Math.min(95, Math.max(10, Math.round(message.progress * 85))));
                    }
                },
            });
            ...
            try {
                ...
                if (file.type === "application/pdf") {
                    ...
                    for (let pageNumber = 1; pageNumber <= total; pageNumber += 1) {
                        setStatus("processing");
                        setProgress(Math.round(((pageNumber - 1) / total) * 80) + 10);
                        ...
                        if (!context) throw new Error("امکان ایجاد تصویر صفحه PDF وجود ندارد.");
    ...
    const downloadWord = async () => {
        ...
        try {
            ...
            const a = document.createElement("a");
            a.href = url;
            a.download = `${fileName.replace(/\.[^.]+$/, "") || "ocr-result"}.docx`;
            a.click();
            URL.revokeObjectURL(url);
        } catch (err) {
            console.error("Word export failed", err);
            setError("ساخت فایل Word انجام نشد.");
        }
    };
            ...
            <div className="mx-auto max-w-6xl px-5 lg:px-8">
                ...
                <div className="mt-10 grid gap-6 lg:grid-cols-[360px_minmax(0,1fr)]">
                    <section className="h-fit rounded-3xl border border-[var(--border)] bg-[var(--surface)] p-5 shadow-sm">
                        ...
                        {fileName && <div className="mt-4 rounded-xl bg-[var(--surface-secondary)] p-3 text-sm font-bold break-all">{fileName}</div>}
