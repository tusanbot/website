...
import { ArrowLeft, ArrowRight, BookOpen, CalendarDays, ChevronLeft, Search, Sparkles, Tag } from "lucide-react";
...
const PAGE_SIZE = 9;
type Post = { id: string; title: string; slug: string; excerpt: string | null; featured_image: string | null; published_at: string | null; category_id: string | null; blog_categories: { name: string; slug: string } | null };
...
export default async function BlogPage({ searchParams }: { searchParams: Promise<{ q?: string; category?: string; page?: string }> }) {
  ...
  const [{ data: categoriesData }, { data: allPostsData }] = await Promise.all([
  ...
  let posts = (allPostsData ?? []) as unknown as Post[];
  ...
  (allPostsData ?? []).forEach((post: any) => categoryCounts.set(post.category_id, (categoryCounts.get(post.category_id) || 0) + 1));
  ...
  return <main dir="rtl" className="blog-hub">
    <style>{`
      .blog-hub{min-height:100vh;background:linear-gradient(180deg,#f3fbf8 0%,#f8fafc 28%,#fff 100%);color:var(--text-primary);padding-bottom:5rem}.blog-wrap{max-width:1280px;margin:auto;padding:0 20px}
