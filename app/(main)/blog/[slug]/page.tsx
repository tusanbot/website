...
async function getPost(slug: string) {
  ...
  const { data } = await supabase.from("blog_posts").select("id,title,slug,excerpt,content,featured_image,meta_title,meta_description,published_at,blog_categories(name,slug),blog_post_services(service_id,services(id,title,slug))").eq("slug", decodeURIComponent(slug).normalize("NFC")).eq("status", "published").maybeSingle();
  return data as any;
}
...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  return { title, description, alternates: { canonical }, openGraph: { type: "article", locale: "fa_IR", title, description, url: canonical, siteName: "کافی نت توسن", publishedTime: post.published_at || undefined } };
}
