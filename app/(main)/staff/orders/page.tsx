...
export default function StaffOrdersPage() {
    ...
    async function load() {
        ...
        const { data, error } = await supabase
            .from("orders")
            .select("id,tracking_code,user_id,service_id,status,price,created_at,updated_at,assignment_status,assigned_staff_id,processing_status,services(title,icon),profiles(full_name,phone)")
            .in("status", ["checking", "processing", "ready"])
            .order("created_at", { ascending: false });
        if (error) setMessage("دریافت سفارش‌ها انجام نشد. لطفاً دوباره تلاش کنید.");
        setOrders(data || []);
        setLoading(false);
    }
    ...
    const filtered = useMemo(() => {
        ...
        if (!q) return orders;
        return orders.filter((order) => [order.tracking_code, order.profiles?.full_name, order.profiles?.phone, order.services?.title].some((v) => String(v || "").toLowerCase().includes(q)));
    }, [orders, search]);
    ...
    return <main dir="rtl" className="min-h-screen page-background p-4 sm:p-6 text-[var(--text)]">
        <div className="mx-auto max-w-7xl space-y-6">
            ...
            {loading ? <GlassPanel className="p-10 text-center text-[var(--text-muted)]">در حال دریافت سفارش‌ها...</GlassPanel> : filtered.length === 0 ? <GlassPanel className="p-10 text-center"><div className="text-5xl mb-4">📭</div><p className="text-[var(--text-muted)]">سفارش قابل نمایش پیدا نشد.</p></GlassPanel> : <div className="grid gap-4 md:grid-cols-2">
                {filtered.map((order) => {
                    const assignedToMe = order.assigned_staff_id === userId;
                    const available = order.assignment_status === "unassigned";
                    return <GlassPanel key={order.id} className="p-5">
                        ...
                        <div className="mt-4 grid grid-cols-2 gap-3 text-sm"><div><span className="text-[var(--text-muted)]">مشتری</span><div className="font-bold mt-1">{order.profiles?.full_name || "---"}</div></div><div><span className="text-[var(--text-muted)]">مبلغ</span><div className="font-bold mt-1">{Number(order.price || 0).toLocaleString("fa-IR")} تومان</div></div><div><span className="text-[var(--text-muted)]">تخصیص</span><div className="font-bold mt-1">{assignedToMe ? "تخصیص به شما" : order.assignment_status === "pending_approval" ? "در انتظار تأیید" : order.assignment_status === "assigned" ? "تخصیص یافته" : "آماده تخصیص"}</div></div><div><span className="text-[var(--text-muted)]">پردازش</span><div className="font-bold mt-1">{order.processing_status || "---"}</div></div></div>
