...
export default function AdminSupportPage() {
    ...
    useEffect(() => {
        ...
        async function init() {
            ...
            if (!mounted) return;
            if (!user) {
                setError("دسترسی به حساب کاربری امکان‌پذیر نیست.");
                setLoading(false);
                return;
            }
            setAdminId(user.id);
    ...
    useEffect(() => {
        ...
        let mounted = true;
        const channel = supabase
            .channel(`support-admin-${selected}`)
    ...
    async function loadConversations() {
        const { data, error: queryError } = await supabase
            ...
            .select("id,user_id,status,created_at,updated_at,profiles(full_name,phone)")
            .eq("status", "open")
            .order("updated_at", { ascending: false });
        if (queryError) {
            setError("بارگذاری گفتگوها انجام نشد. لطفاً دوباره تلاش کنید.");
            setLoading(false);
            return;
        }
    ...
    async function loadMessages(conversationId: string) {
        ...
        const { data, error: queryError } = await supabase
            ...
            .limit(PAGE_SIZE + 1);
        if (queryError) {
            setError("بارگذاری پیام‌ها انجام نشد. لطفاً دوباره تلاش کنید.");
    ...
    async function loadOlderMessages() {
        ...
        const { data, error: queryError } = await supabase
            ...
            .limit(PAGE_SIZE + 1);
        if (queryError) {
            setError("بارگذاری پیام‌های قدیمی انجام نشد.");
            setMessagesLoading(false);
            return;
        }
        const rows = (data || []) as Message[];
        const page = rows.slice(0, PAGE_SIZE).reverse();
        setMessages((current) => [...page, ...current.filter((item) => !page.some((older) => older.id === item.id))]);
        setHasMore(rows.length > PAGE_SIZE);
        setOldestAt(page[0]?.created_at ?? oldestAt);
        setMessagesLoading(false);
    }
