...
export default function AdminOrdersPage() {
    ...
    async function loadOrders() {
        ...
        try {
            ...
            if (!user) {
                setError(
                    "برای مشاهده سفارش‌ها باید وارد حساب کاربری شوید."
                );
                return;
            }
            ...
            if (profile?.role !== "admin") {
                setError(
                    "شما دسترسی لازم برای مشاهده سفارش‌ها را ندارید."
                );
                return;
            }
    ...
    return (
        <div
            dir="rtl"
            className="min-h-screen page-background text-[var(--text)] p-6 transition-colors duration-300"
        >
                    ...
                    <>
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
                            <TusanStatCard
                                ...
                                icon={<TusanIcon name="📋" size={28} className="text-[var(--primary)]" />}
                            />

                            <TusanStatCard
                                title="در حال بررسی"
                                value={(statusCounts.checking || 0).toLocaleString("fa-IR")}
                                icon="🔎"
                            />

                            <TusanStatCard
                                title="در حال انجام"
                                value={(statusCounts.processing || 0).toLocaleString("fa-IR")}
                                icon="⚙️"
                            />

                            <TusanStatCard
                                title="تکمیل شده"
                                value={(statusCounts.completed || 0).toLocaleString("fa-IR")}
                                icon="✅"
                            />
                        </div>
                        ...
                        {orders.length === 0 && (
                            <GlassPanel className="p-10 text-center">
                                ...
                                <p className="text-[var(--text-muted)] mt-2">
                                    سفارش‌های جدید مشتریان در این بخش نمایش داده می‌شوند.
                                </p>
