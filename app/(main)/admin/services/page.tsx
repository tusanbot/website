...
export default function AdminServicesPage() {
  ...
  return <div dir="rtl" className="min-h-screen page-background text-[var(--text)] p-6 transition-colors duration-300"><div className="max-w-6xl mx-auto">
    <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-6"><SectionHeader title="مدیریت خدمات" description="مدیریت خدمات، وضعیت نمایش، قیمت و فرم‌های هر خدمت" /><Link href="/admin/services/new-service"><TusanButton icon={<span>＋</span>}>خدمت جدید</TusanButton></Link></div>
    <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6"><TusanStatCard title="کل خدمات" value={services.length.toLocaleString("fa-IR")} icon="🛠️" /><TusanStatCard title="فعال" value={services.filter(s => s.is_active).length.toLocaleString("fa-IR")} icon="✅" /><TusanStatCard title="غیرفعال" value={services.filter(s => !s.is_active).length.toLocaleString("fa-IR")} icon="⛔" /><TusanStatCard title="دارای قیمت" value={services.filter(s => Number(s.price || 0) > 0).length.toLocaleString("fa-IR")} icon="💳" /></div>
    <TusanTable columns={[{key:"icon",title:"آیکون",width:"80px",align:"center"},{key:"title",title:"عنوان"},{key:"category",title:"دسته‌بندی"},{key:"price",title:"قیمت",align:"left"},{key:"status",title:"وضعیت",align:"center"},{key:"actions",title:"عملیات",align:"left"}]} rows={services.map(service => ({
