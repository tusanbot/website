...
export default function EditServicePage() {
  ...
  const [serviceType, setServiceType] = useState<"normal" | "parent">("normal");
  const [parentServiceId, setParentServiceId] = useState(""); const [parentFormId, setParentFormId] = useState("");
  ...
  async function loadService() {
    ...
    try {
      ...
      setTitle(service.title || ""); setCategory(service.category || ""); setDescription(service.description || "");
      setPrice(service.price == null ? "" : String(service.price)); setIcon(service.icon || ""); setIsActive(service.is_active ?? true);
      setServiceType(service.service_type === "parent" ? "parent" : "normal");
      setParentServiceId(service.parent_service_id || ""); setParentFormId(service.parent_form_id || "");
  ...
  function handleParentServiceChange(id: string) {
    setParentServiceId(id);
    const selected = parentServices.find(item => item.id === id);
    if (selected?.parent_form_id) setParentFormId(selected.parent_form_id);
  }
  ...
  async function updateService(e: React.FormEvent) {
    ...
    try {
      ...
      const { error: updateError } = await supabase.from("services").update({
        title: title.trim(), category: category.trim() || null, description: description.trim() || null, price: parsedPrice,
        icon: icon.trim() || null, is_active: isActive, form_schema: formSchema,
        parent_service_id: serviceType === "normal" ? (parentServiceId || null) : null,
  ...
  return <div dir="rtl" className="min-h-screen page-background p-6 text-[var(--text)]"><div className="max-w-5xl mx-auto space-y-6">
    ...
    <GlassPanel className="p-6"><form onSubmit={updateService} className="space-y-6">
      <TusanCard className="p-5 space-y-5"><div className="grid md:grid-cols-2 gap-5">
      ...
      </div><div><label className="block font-bold mb-2">توضیحات خدمت</label><textarea value={description} onChange={e => setDescription(e.target.value)} rows={4} className="w-full rounded-xl border border-[var(--border)] bg-[var(--surface)] px-4 py-3" /></div>
      <label className="flex items-center gap-3"><input type="checkbox" checked={isActive} onChange={e => setIsActive(e.target.checked)} className="w-5 h-5 accent-[#09967C]" /><span className="font-bold">خدمت فعال باشد</span></label></TusanCard>

      {serviceType === "normal" && <TusanCard className="p-5 space-y-5"><div><h3 className="font-bold text-lg">اتصال به ساختار مادر</h3><p className="text-sm text-[var(--muted)] mt-1">فرم مادر موجود را انتخاب کنید؛ نیازی به ساخت مجدد آن نیست.</p></div>
        <div className="grid md:grid-cols-2 gap-4"><div><label className="block text-sm font-bold mb-2">خدمت مادر</label><select value={parentServiceId} onChange={e => handleParentServiceChange(e.target.value)} className="w-full rounded-xl border border-[var(--border)] bg-[var(--surface)] px-4 py-3"><option value="">بدون خدمت مادر</option>{parentServices.map(item => <option key={item.id} value={item.id}>{item.title}</option>)}</select></div>
        <div><label className="block text-sm font-bold mb-2">فرم مادر</label><select value={parentFormId} onChange={e => setParentFormId(e.target.value)} className="w-full rounded-xl border border-[var(--border)] bg-[var(--surface)] px-4 py-3"><option value="">بدون فرم مادر</option>{parentForms.map(form => <option key={form.id} value={form.id}>{form.title}</option>)}</select></div></div>
      </TusanCard>}
