...
export default function EditServicePage() {
  ...
  async function loadService() {
    ...
    try {
      ...
      setTitle(service.title || ""); setSlug(service.slug || ""); setCategory(service.category || ""); setDescription(service.description || ""); setPrice(service.price == null ? "" : String(service.price)); setIcon(service.icon || ""); setIsActive(service.is_active ?? true);
  ...
  async function updateService(e: React.FormEvent) {
    ...
    try {
      ...
      const { error: updateError } = await supabase.from("services").update({ title: title.trim(), slug: slug.trim() || null, category: category.trim() || null, description: description.trim() || null, price: parsedPrice, icon: icon.trim() || null, is_active: isActive, meta_title: metaTitle.trim() || null, meta_description: metaDescription.trim() || null, seo_keywords: seoKeywords, form_schema: formSchema, parent_service_id: serviceType === "normal" ? (parentServiceId || null) : null, parent_form_id: parentFormId || null }).eq("id", serviceId);
