      ...
      <GlassPanel className="p-6">
        ...
        <form onSubmit={submit} className="space-y-6">
          <TusanCard className="p-5 space-y-5">
            <div><h2 className="text-lg font-bold">اطلاعات خدمت</h2><p className="text-sm text-gray-500 mt-1">اطلاعات خدمت مستقل از فرم مادر ذخیره می‌شود.</p></div>
            <div className="grid md:grid-cols-2 gap-4">
              <div><label className="block text-sm font-bold mb-2">عنوان خدمت *</label><TusanInput value={title} onChange={e => setTitle(e.target.value)} /></div>
              <div><label className="block text-sm font-bold mb-2">دسته‌بندی</label><TusanInput value={category} onChange={e => setCategory(e.target.value)} /></div>
              <div><label className="block text-sm font-bold mb-2">قیمت (تومان)</label><TusanInput type="number" min="0" value={price} onChange={e => setPrice(e.target.value)} /></div>
              <div><label className="block text-sm font-bold mb-2">آیکون</label><TusanInput value={icon} onChange={e => setIcon(e.target.value)} /></div>
            </div>
          ...
          <TusanCard className="p-5 space-y-5">
            <label className="flex items-start gap-3 cursor-pointer"><input id="with-parent" type="checkbox" checked={withParent} onChange={e => setWithParent(e.target.checked)} className="mt-1 w-5 h-5 accent-[#09967C]" /><span><div className="font-bold">برای این خدمت فرم مادر ایجاد شود</div><div className="text-sm text-gray-500 mt-1">اختیاری است.</div></span></label>
            {withParent && <div className="border-t pt-5 space-y-5">
              <div className="rounded-xl bg-[#09967C]/5 border border-[#09967C]/20 p-4 text-sm">فرم مادر با عنوان خدمت ایجاد می‌شود. فرم فرزند نیز اختیاری است.</div>
              <div className="grid md:grid-cols-2 gap-4">
                <div><label className="block text-sm font-bold mb-2">عنوان فرم فرزند (اختیاری)</label><TusanInput value={childTitle} onChange={e => setChildTitle(e.target.value)} placeholder="مثلاً سایپا" /></div>
              ...
              <div><label className="block text-sm font-bold mb-2">توضیحات فرم فرزند</label><TusanInput value={childDescription} onChange={e => setChildDescription(e.target.value)} /></div>
