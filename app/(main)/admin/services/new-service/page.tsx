...
export default function NewServicePage() {
  ...
  function resetForMode(next: "normal" | "parent") { setMode(next); setError(""); setTitle(""); setCategory(""); setDescription(""); setPrice(""); setIcon(""); setIsPopular(false); setSlug(""); setMetaTitle(""); setMetaDescription(""); setSeoKeywords([]); setFormSchema([]); setUseParent(false); setParentServiceId(""); setParentFormId(""); }
  ...
  async function submit(e: React.FormEvent) {
    ...
    try {
      ...
      if (mode === "parent") {
        const { data: service, error: serviceError } = await supabase.from("services").insert({ title: title.trim(), slug: slug.trim() || null, category: category.trim() || null, description: description.trim() || null, price: 0, icon: icon.trim() || null, is_active: isActive, is_popular: isPopular, meta_title: metaTitle.trim() || null, meta_description: metaDescription.trim() || null, seo_keywords: seoKeywords, form_schema: [], parent_service_id: null, parent_form_id: null, service_type: "parent" }).select("id").single();
        ...
        const { data: service, error: serviceError } = await supabase.from("services").insert({ title: title.trim(), slug: slug.trim() || null, category: category.trim() || null, description: description.trim() || null, price: servicePrice, icon: icon.trim() || null, is_active: isActive, is_popular: isPopular, meta_title: metaTitle.trim() || null, meta_description: metaDescription.trim() || null, seo_keywords: seoKeywords, form_schema: formSchema, parent_service_id: useParent ? (parentServiceId || null) : null, parent_form_id: useParent ? (parentFormId || null) : null, service_type: "normal" }).select("id").single();
