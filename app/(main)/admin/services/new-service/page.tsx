...
export default function NewServicePage() {
  ...
  async function loadParentData() {
    ...
    } catch (err: any) { setError(err?.message || "دریافت فرم‌های مادر انجام نشد."); }
  ...
  function resetForMode(next: "normal" | "parent") { setMode(next); setError(""); setTitle(""); setCategory(""); setDescription(""); setPrice(""); setIcon(""); setFormSchema([]); setUseParent(false); setParentServiceId(""); setParentFormId(""); }
  ...
  const applyAi = (data: Record<string, unknown>) => {
    ...
    if (data.icon) setIcon(String(data.icon));
  ...
  async function submit(e: React.FormEvent) {
    ...
    try {
      ...
      if (mode === "parent") {
        const { data: service, error: serviceError } = await supabase.from("services").insert({ title: title.trim(), category: category.trim() || null, description: description.trim() || null, price: 0, icon: icon.trim() || null, is_active: isActive, form_schema: [], parent_service_id: null, parent_form_id: null, service_type: "parent" }).select("id").single();
        ...
        const { error: linkError } = await supabase.from("services").update({ parent_form_id: form.id }).eq("id", service.id);
        ...
        const { data: service, error: serviceError } = await supabase.from("services").insert({ title: title.trim(), category: category.trim() || null, description: description.trim() || null, price: servicePrice, icon: icon.trim() || null, is_active: isActive, form_schema: formSchema, parent_service_id: useParent ? (parentServiceId || null) : null, parent_form_id: useParent ? (parentFormId || null) : null, service_type: "normal" }).select("id").single();
