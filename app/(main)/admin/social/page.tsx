...
import { ArrowRight, CheckCircle2, Package, RefreshCw, Settings2, Wifi, WifiOff } from "lucide-react";
    ...
    async function syncServices() {
        ...
        try {
            ...
            if (!session?.access_token) {
                setSyncResult({ error: "جلسه کاربری معتبر نیست. دوباره وارد حساب مدیریت شوید." });
                return;
            }
            ...
            window.localStorage.setItem("tusan_social_last_sync", timestamp);
            await load();
        } catch (error) {
    ...
    return (
        <main dir="rtl" className="min-h-screen page-background text-[var(--text)] p-4 sm:p-6">
            <div className="max-w-7xl mx-auto space-y-6">
                ...
                {syncResult && (
                    <div className={`rounded-2xl border p-4 ${syncResult.error ? "border-red-200 bg-red-50 text-red-800" : "border-green-200 bg-green-50 text-green-800"}`}>
                        {syncResult.error ? <div className="font-bold">همگام‌سازی ناموفق بود: {syncResult.error}</div> : <div className="space-y-1"><div className="font-bold">همگام‌سازی با موفقیت انجام شد.</div><div className="text-sm">دریافت‌شده: {Number(syncResult.received || 0).toLocaleString("fa-IR")} · ثبت/به‌روزرسانی: {Number(syncResult.synced || 0).toLocaleString("fa-IR")} · ردشده: {Number(syncResult.skipped || 0).toLocaleString("fa-IR")}</div></div>}
                    </div>
                )}
                ...
                <section className="rounded-3xl border border-[var(--border)] bg-[var(--surface)] overflow-hidden">
                    <div className="p-5 border-b border-[var(--border)]"><h2 className="font-black text-xl">کاتالوگ سرویس‌ها</h2><p className="text-sm text-[var(--text-muted)] mt-1">سرویس‌ها از FJPanel همگام می‌شوند و در این جدول قابل مشاهده هستند.</p></div>
