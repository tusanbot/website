...
export default async function ServicePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [{ data: related }, { data: children }, { data: parent }] = await Promise.all([
    ...
    supabase.from("services").select("id,title,slug,icon,description").eq("is_active", true).eq("parent_service_id", service.id).order("created_at", { ascending: false }).limit(12),
  ...
  return <>
    ...
    <div dir="rtl" className="max-w-3xl mx-auto px-6 pt-5"><nav aria-label="مسیر صفحه" className="text-sm text-[var(--text-muted)]"><Link href="/" className="hover:underline">خانه</Link><span className="mx-2">/</span><Link href="/services" className="hover:underline">خدمات</Link><span className="mx-2">/</span>{service.category && <><Link href={`/services?category=${encodeURIComponent(service.category)}`} className="hover:underline">{service.category}</Link><span className="mx-2">/</span></>}<span className="font-medium text-[var(--text)]" aria-current="page">{service.title}</span></nav></div>
    ...
    <section dir="rtl" className="max-w-3xl mx-auto px-6 pt-5 pb-2" aria-labelledby="service-guide-title"><div className="rounded-2xl border bg-white p-6 shadow-sm"><h1 id="service-guide-title" className="text-2xl font-bold">{service.title}</h1>{service.description?.trim() && <div className="mt-4 text-[var(--text-muted)] leading-8"><p>{service.description.trim()}</p></div>}<div className="mt-6 grid gap-5 sm:grid-cols-2"><div><h2 className="font-bold text-lg">نحوه ثبت درخواست</h2><ol className="mt-2 list-decimal pr-5 space-y-1 text-sm leading-7 text-[var(--text-muted)]"><li>اطلاعات موردنیاز این خدمت را در فرم مربوط وارد کنید.</li><li>اطلاعات واردشده را پیش از ارسال بررسی و تأیید کنید.</li><li>پس از ثبت، وضعیت سفارش را از مسیر پیگیری سفارش بررسی کنید.</li></ol></div><div><h2 className="font-bold text-lg">اطلاعات موردنیاز</h2>{fieldLabels.length > 0 ? <ul className="mt-2 list-disc pr-5 space-y-1 text-sm leading-7 text-[var(--text-muted)]">{fieldLabels.map((label) => <li key={label}>{label}</li>)}</ul> : <p className="mt-2 text-sm leading-7 text-[var(--text-muted)]">اطلاعات موردنیاز این خدمت هنگام تکمیل فرم نمایش داده می‌شود.</p>}</div></div><div className="mt-6 rounded-xl bg-[var(--surface-muted)] p-4"><h2 className="font-bold">نکته مهم</h2><p className="mt-1 text-sm leading-7 text-[var(--text-muted)]">قبل از ثبت نهایی، اطلاعات و فایل‌های واردشده را با دقت بررسی کنید. در صورت وجود شرایط اختصاصی برای این خدمت، موارد لازم در فرم ثبت درخواست نمایش داده می‌شود.</p></div></div></section>
