  ...
  return <>
    ...
    <section dir="rtl" className="max-w-3xl mx-auto px-6 pt-5 pb-2" aria-labelledby="service-guide-title">
      <div className="rounded-2xl border bg-white p-6 shadow-sm">
        ...
        {service.description?.trim() && <div className="mt-4 text-[var(--text-muted)] leading-8"><p>{service.description.trim()}</p></div>}
        <div className="mt-6 grid gap-5 sm:grid-cols-2">
          ...
          <div>
            ...
            {fieldLabels.length > 0 ? <ul className="mt-2 list-disc pr-5 space-y-1 text-sm leading-7 text-[var(--text-muted)]">{fieldLabels.map((label) => <li key={label}>{label}</li>)}</ul> : <p className="mt-2 text-sm leading-7 text-[var(--text-muted)]">اطلاعات موردنیاز این خدمت هنگام تکمیل فرم نمایش داده می‌شود.</p>}
          </div>
        </div>
        <div className="mt-6 rounded-xl bg-[var(--surface-muted)] p-4">
          <h2 className="font-bold">نکته مهم</h2>
          <p className="mt-1 text-sm leading-7 text-[var(--text-muted)]">قبل از ثبت نهایی، اطلاعات و فایل‌های واردشده را با دقت بررسی کنید. در صورت وجود شرایط اختصاصی برای این خدمت، موارد لازم در فرم ثبت درخواست نمایش داده می‌شود.</p>
        </div>
      </div>
    </section>
