...
type ChildForm = { id: string; title: string; description: string | null; schema: any[]; sort_order: number };
...
export default function ServiceOrderPage() {
    ...
    async function loadService() {
        ...
        try {
            ...
            if (parentError) { console.warn("Form hierarchy is unavailable:", parentError.message); setHasParentForm(false); return; }
            if (!parentForm) { setHasParentForm(false); return; }
            setHasParentForm(true);
            const { data: children, error: childrenError } = await supabase.from("custom_forms").select("id,title,description,schema,sort_order").eq("service_id", serviceId).eq("parent_form_id", parentForm.id).eq("form_type", "normal").eq("is_public", true).order("sort_order", { ascending: true }).order("created_at", { ascending: true });
            if (childrenError) throw new Error(childrenError.message);
            setChildForms((children || []).map((form: any) => ({ ...form, schema: normalizeSchema(form.schema) })));
    ...
    async function submitOrder(formData: Record<string, any>) {
        ...
        try {
            ...
            const { data: order, error: orderError } = await supabase.from("orders").insert({ user_id: user.id, service_id: service.id, form_id: selectedForm?.id || null, tracking_code: generateTrackingCode(), status: "registered", form_data: { ...formData }, price: service.price || 0 }).select("id,tracking_code").single();
