...
const MAX_PROVIDER_RATE = 1_000_000;
const BLOCKED_TEXT = ["موقتا غیرفعال", "موقتا غیر فعال", "موقتاً غیرفعال", "موقتاً غیر فعال", "در حال تست", "از این دسته سفارش ندهید", "temporarily disabled", "do not order", "testing"];
...
function parseRate(value: unknown) {
  ...
  return Number.isFinite(parsed) && parsed > 0 && parsed <= MAX_PROVIDER_RATE ? parsed : null;
}

function hasBlockedText(service: FJPanelService) {
  const text = normalize(`${service.name ?? ""} ${service.category ?? ""}`);
  return BLOCKED_TEXT.some((phrase) => text.includes(normalize(phrase)));
}

function validateProviderService(service: FJPanelService) {
  if (!service?.service || !service.name?.trim()) return false;
  if (hasBlockedText(service)) return false;
  const rate = parseRate(service.rate);
  const min = parsePositiveInteger(service.min, 1);
  const max = parsePositiveInteger(service.max, min);
  if (rate == null || min <= 0 || max < min || max > 100_000_000) return false;
  return true;
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    for (const providerService of providerServices) {
      ...
      if (!validateProviderService(providerService)) {
        skipped += 1;
        if (providerId) {
          const { data: existing } = await admin.from("social_services").select("id").eq("provider", "fjpanel").eq("provider_service_id", providerId).maybeSingle();
          if (existing?.id) {
            await admin.from("social_services").update({ is_active: false }).eq("id", existing.id);
            disabled += 1;
          }
        }
        continue;
      ...
      const { data: existing, error: existingError } = await admin.from("social_services").select("id,name").eq("provider", "fjpanel").eq("provider_service_id", providerId).maybeSingle();
      ...
      const maxQuantity = parsePositiveInteger(providerService.max, minQuantity);
      const providerRate = parseRate(providerService.rate);
      if (providerRate == null || maxQuantity < minQuantity) {
        skipped += 1;
        if (existing?.id) {
          await admin.from("social_services").update({ is_active: false }).eq("id", existing.id);
          disabled += 1;
      ...
      const { error } = await admin.from("social_services").upsert({
        ...
        provider_name: providerService.name.trim(),
    ...
    return NextResponse.json({ success: true, provider: "fjpanel", received: providerServices.length, synced: createdOrUpdated, skipped, disabled, syncedAt: new Date().toISOString() });
  } catch (error) {
