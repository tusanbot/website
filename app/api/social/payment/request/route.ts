...
export async function POST(request: NextRequest) {
    try {
        ...
        const { data: order, error } = await admin.from("social_orders")
            .select("id,user_id,tracking_code,price,status,payment_track_id,payment_provider,updated_at")
        ...
        if (order.status === "awaiting_payment" && !order.payment_track_id) {
            if (order.updated_at && order.updated_at > staleBefore) {
                return NextResponse.json({ error: "درخواست پرداخت دیگری در حال ایجاد است. چند لحظه بعد دوباره تلاش کنید." }, { status: 409 });
            }
        } else if (!["pending", "failed", "cancelled"].includes(order.status)) {
            return NextResponse.json({ error: "این سفارش در وضعیت قابل پرداخت نیست." }, { status: 409 });
        }

        // Claim the order before calling the gateway. This makes concurrent requests
        // mutually exclusive and prevents multiple Zibal transactions for one order.
        let claimQuery = admin.from("social_orders")
            .update({
        ...
        if (order.status === "awaiting_payment") {
            claimQuery = claimQuery.eq("status", "awaiting_payment").lt("updated_at", staleBefore) as typeof claimQuery;
        } else {
            claimQuery = claimQuery.in("status", ["pending", "failed", "cancelled"]) as typeof claimQuery;
        }

        const { data: claimedOrder, error: claimError } = await claimQuery.select("id").maybeSingle();

        if (claimError) {
            console.error("[social/payment/request] payment claim failed", claimError);
            return NextResponse.json({ error: "قفل‌کردن درخواست پرداخت ناموفق بود." }, { status: 500 });
        }

        if (!claimedOrder) {
            return NextResponse.json({ error: "درخواست پرداخت دیگری در حال پردازش است. چند لحظه بعد دوباره تلاش کنید." }, { status: 409 });
        }
        ...
        try {
            ...
            const { error: updateError } = await admin.from("social_orders").update({
                ...
                paid_at: null,
                admin_note: null,
            }).eq("id", order.id).eq("status", "awaiting_payment").is("payment_track_id", null);
