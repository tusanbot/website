...
export async function POST(request: NextRequest) {
    ...
    try {
        ...
        const { data: order, error: orderError } = await admin
            .from("social_orders")
            .select("id, service_id, link, quantity, status, provider, provider_order_id")
            .eq("id", orderId)
            .maybeSingle();
        ...
        if (order.provider !== "fjpanel") {
            return NextResponse.json({ error: "ارائه‌دهنده این سفارش پشتیبانی نمی‌شود." }, { status: 400 });
        }
        ...
        if (!claimed) {
            return NextResponse.json({ error: "این سفارش قبلاً در حال پردازش یا ارسال است." }, { status: 409 });
        }
        ...
        try {
            ...
            return NextResponse.json({
                success: true,
                providerOrderId: String(provider.order),
            });
        } catch (providerError) {
            await admin
                .from("social_orders")
                .update({
                    ...
                    admin_note: providerError instanceof Error
                        ...
                        : "ارسال به ارائه‌دهنده ناموفق بود.",
                })
                .eq("id", order.id)
        ...
        return NextResponse.json({
            error: error instanceof Error
                ? error.message
                : "ارسال سفارش به ارائه‌دهنده ناموفق بود.",
        }, { status: 500 });
    }
}
