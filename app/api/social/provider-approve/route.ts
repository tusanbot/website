...
export async function POST(request: NextRequest) {
    ...
    try {
        ...
        if (error) {
            console.error("[social/provider-approve] order lookup failed", error);
        ...
        if (order.admin_approved || order.provider_order_id) {
            return NextResponse.json({ error: "این سفارش قبلاً تأیید یا ارسال شده است." }, { status: 409 });
        }
        if (order.provider !== "fjpanel") {
            return NextResponse.json({ error: "سرویس‌دهنده این سفارش پشتیبانی نمی‌شود." }, { status: 409 });
        }
        ...
        if (serviceError) {
            console.error("[social/provider-approve] service lookup failed", serviceError);
            return NextResponse.json({ error: "دریافت سرویس ارائه‌دهنده ناموفق بود." }, { status: 500 });
        }

        if (!service?.is_active || !service.provider_service_id) {
            return NextResponse.json({ error: "سرویس ارائه‌دهنده برای این سفارش معتبر یا فعال نیست." }, { status: 409 });
        }

        // Atomically claim the paid order before contacting the provider.
        // This prevents two concurrent admin requests from submitting the
        // same paid order twice.
        ...
        if (claimError) {
            console.error("[social/provider-approve] claim failed", claimError);
            return NextResponse.json({ error: "قفل‌کردن سفارش برای پردازش ناموفق بود." }, { status: 500 });
        }

        if (!claimed) {
            return NextResponse.json({ error: "این سفارش هم‌اکنون در حال پردازش یا قبلاً ارسال شده است." }, { status: 409 });
        }

        try {
            const provider = await addFJPanelOrder(
            ...
            await admin
                ...
                .update({
                    status: "paid",
                    admin_note: "تأیید انجام شد اما ارسال به FJPanel ناموفق بود.",
                })
                .eq("id", order.id)
            ...
            return NextResponse.json({
                error: "تأیید انجام شد اما ارسال به FJPanel ناموفق بود.",
            }, { status: 502 });
        }
    } catch (error) {
