...
export async function POST(request: NextRequest) {
    try {
        ...
        if (!cancellable) {
            return NextResponse.json({ error: "این سفارش دیگر قابل لغو نیست." }, { status: 409 });
        }
        if (order.provider_order_id) {
            return NextResponse.json({ error: "سفارش به سرویس‌دهنده ارسال شده و قابل لغو نیست." }, { status: 409 });
        }

        const { error: updateError } = await admin.from("social_orders")
            .update({ status: "cancelled", admin_note: "لغو توسط کاربر" })
            .eq("id", order.id)
            .eq("user_id", userId)
