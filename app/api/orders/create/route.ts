...
function normalizeSchema(value: unknown): FormSchema { let parsed=value; if(typeof parsed==='string'){try{parsed=JSON.parse(parsed)}catch{parsed=[]}} if(Array.isArray(parsed)) return {fields:parsed as FormSchema['fields']}; if(parsed&&typeof parsed==='object'&&Array.isArray((parsed as any).fields)) return parsed as FormSchema; return {fields:[]}; }
function normalizeRules(value: unknown): PricingRule[] { if(typeof value==='string'){try{value=JSON.parse(value)}catch{value=[]}} return Array.isArray(value)?value as PricingRule[]:[]; }
function mergeSchemas(parent: FormSchema, child: FormSchema): FormSchema { const seen=new Set<string>(); return {...child,fields:[...parent.fields,...child.fields].filter((f:any)=>{if(!f||typeof f!=='object')return false;const k=String(f.name||f.id||'').trim().toLowerCase();if(!k||seen.has(k))return false;seen.add(k);return true})}; }
function buildPricingData(fields: FormField[], validated: Record<string, unknown>): Record<string, unknown> { const out={...validated}; for(const field of fields){const value=validated[field.id];if(value===undefined)continue;out[field.name]=field.type==='repeatable'&&Array.isArray(value)&&field.fields?.length?value.map(row=>row&&typeof row==='object'?buildPricingData(field.fields||[],row as Record<string,unknown>):row):value;} return out; }
function generateTrackingCode(){return `TUS-${Date.now().toString().slice(-6)}-${Math.floor(100000+Math.random()*900000)}`;}
async function readBoundedJson(request:Request){const len=request.headers.get('content-length');if(len&&Number.isFinite(Number(len))&&Number(len)>MAX_ORDER_BODY_BYTES)throw new Error('REQUEST_BODY_TOO_LARGE');const body=await request.arrayBuffer();if(body.byteLength>MAX_ORDER_BODY_BYTES)throw new Error('REQUEST_BODY_TOO_LARGE');return JSON.parse(new TextDecoder().decode(body));}
...
export async function POST(request:Request):Promise<Response>{try{
 ...
 if(idempotencyKey){const {data:existing,error}=await db.from('orders').select('id,tracking_code,price,form_version_id').eq('user_id',user.id).eq('idempotency_key',idempotencyKey).maybeSingle();if(error)throw new Error(error.message);if(existing)return NextResponse.json({order:existing,idempotent:true});}
 const {data:service,error:serviceError}=await db.from('services').select('id,title,price,form_schema,is_active,parent_service_id,parent_form_id,pricing_rules').eq('id',body.serviceId).eq('is_active',true).maybeSingle();if(serviceError)throw new Error(serviceError.message);if(!service)return NextResponse.json({error:'خدمت انتخاب‌شده در دسترس نیست.'},{status:404});
