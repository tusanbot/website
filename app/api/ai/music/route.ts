...
export async function POST(request: NextRequest) {
  try {
    ...
    const rateLimitResponse = await checkRateLimit({ scope: "ai:music", request, limit: 4, windowSeconds: 60 });
    ...
    const key = await getProfileApiKey(session.profile.id, "music");
    const model = await getAiCapabilityModel(session.profile.id, "music", key, session.profile.model_config);
    ...
    const response = await fetch(`${base}/models/${encodeURIComponent(model)}:generateContent`, { method: "POST", headers: { "Content-Type": "application/json", "x-goog-api-key": key }, body: JSON.stringify({ contents: [{ parts }], generationConfig: { responseModalities: ["AUDIO", "TEXT"] } }), cache: "no-store", signal: AbortSignal.timeout(180000) });
    const data = await response.json();
    if (!response.ok) return NextResponse.json({ error: data?.error?.message || "خطا از مدل موسیقی.", model }, { status: 502 });
    ...
    return NextResponse.json({ url: `data:${payload.mimeType || payload.mime_type || "audio/mpeg"};base64,${payload.data}`, text, model });
  } catch (error) {
    ...
    if (error instanceof Error && error.message === "GEMINI_MUSIC_MODEL_UNAVAILABLE") return NextResponse.json({ error: "این کلید Gemini به مدل تولید موسیقی دسترسی ندارد." }, { status: 503 });
