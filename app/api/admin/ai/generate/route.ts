...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (!apiKey) return NextResponse.json({ error: "Credential هوش مصنوعی داخلی توسن تنظیم نشده است." }, { status: 503 });
    ...
    const result = await generateWithGeminiApiKey(apiKey, prompt, process.env.TUSAN_GEMINI_MODEL || "gemini-2.5-flash");
    ...
    return NextResponse.json({ data, model: result.model, source: "tusan" });
  } catch (error) {
    const message = error instanceof Error ? error.message : "AI generation failed";
    if (message === "GEMINI_AUTH") return NextResponse.json({ error: "کلید Gemini داخلی معتبر نیست یا دسترسی لازم را ندارد." }, { status: 401 });
    if (message === "GEMINI_RATE_LIMIT") return NextResponse.json({ error: "سهمیه یا محدودیت درخواست Gemini داخلی پر شده است." }, { status: 429 });
