...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const model = process.env.TUSAN_GEMINI_MODEL || "gemini-2.5-flash";
    if (!apiKey) return NextResponse.json({ error: "Credential هوش مصنوعی داخلی تنظیم نشده است." }, { status: 503 });

    const rateLimitResponse = await checkRateLimit({ scope: "admin-ai-internal", request, userId: admin.id, limit: 30, windowSeconds: 3600 });
