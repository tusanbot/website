-- Social Services installation migration
-- Run this once in Supabase SQL Editor for the project's production database.
-- Safe to re-run: tables, indexes, policies, triggers and seed rows use IF NOT EXISTS / upsert semantics.
...
drop policy if exists "social platforms admin write" on public.social_platforms;
drop policy if exists "social categories admin write" on public.social_categories;
drop policy if exists "social services admin write" on public.social_services;
drop policy if exists "social orders admin update" on public.social_orders;

create policy "social platforms public read active" on public.social_platforms for select
using (is_active = true or exists (select 1 from public.profiles p where p.id = auth.uid() and p.role = 'admin'));
create policy "social categories public read active" on public.social_categories for select
using (is_active = true or exists (select 1 from public.profiles p where p.id = auth.uid() and p.role = 'admin'));
create policy "social services public read active" on public.social_services for select
using (is_active = true or exists (select 1 from public.profiles p where p.id = auth.uid() and p.role = 'admin'));
create policy "social orders user read own" on public.social_orders for select
using (auth.uid() = user_id or exists (select 1 from public.profiles p where p.id = auth.uid() and p.role = 'admin'));
create policy "social orders user insert own" on public.social_orders for insert
with check (auth.uid() = user_id);
create policy "social platforms admin write" on public.social_platforms for all
using (exists (select 1 from public.profiles p where p.id = auth.uid() and p.role = 'admin'))
with check (exists (select 1 from public.profiles p where p.id = auth.uid() and p.role = 'admin'));
create policy "social categories admin write" on public.social_categories for all
using (exists (select 1 from public.profiles p where p.id = auth.uid() and p.role = 'admin'))
with check (exists (select 1 from public.profiles p where p.id = auth.uid() and p.role = 'admin'));
create policy "social services admin write" on public.social_services for all
using (exists (select 1 from public.profiles p where p.id = auth.uid() and p.role = 'admin'))
with check (exists (select 1 from public.profiles p where p.id = auth.uid() and p.role = 'admin'));
create policy "social orders admin update" on public.social_orders for update
using (exists (select 1 from public.profiles p where p.id = auth.uid() and p.role = 'admin'))
with check (exists (select 1 from public.profiles p where p.id = auth.uid() and p.role = 'admin'));
...
create trigger social_platforms_updated_at before update on public.social_platforms for each row execute function public.set_social_updated_at();
drop trigger if exists social_categories_updated_at on public.social_categories;
create trigger social_categories_updated_at before update on public.social_categories for each row execute function public.set_social_updated_at();
drop trigger if exists social_services_updated_at on public.social_services;
create trigger social_services_updated_at before update on public.social_services for each row execute function public.set_social_updated_at();
drop trigger if exists social_orders_updated_at on public.social_orders;
create trigger social_orders_updated_at before update on public.social_orders for each row execute function public.set_social_updated_at();

insert into public.social_platforms (name, slug, icon, description, sort_order) values
