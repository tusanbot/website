...
export default function NewServicePage() {
  ...
  async function loadParentServices() {
    ...
    try {
      // parent_service_id=null به‌تنهایی برای تشخیص خدمت مادر کافی نیست؛
      // چون تمام خدمات مستقل نیز parent_service_id=null دارند. منبع معتبر
      // فعلی برای تشخیص مادر، custom_forms.form_type=parent است.
      const { data: parentForms, error: formsError } = await supabase
        .from("custom_forms")
        .select("service_id")
        .eq("form_type", "parent")
      ...
      const ids = Array.from(
        new Set(
          (parentForms || [])
            .map((item: { service_id: string | null }) => item.service_id)
